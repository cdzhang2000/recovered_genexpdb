...
sub displayAccessions
{
	...
	if (     $gdb::webUtil::frmData{ginfo}
		 and $gdb::webUtil::frmData{ginfo} =~ /^accessions/ )
	{
	...
	} else
	{
		...
		{
			print
qq{<div class="mn2" style="border-top:1px solid #C3CCD3;"><span onclick="da('accessions');" onmouseover="mm(this,'accessions');" onmouseout="return nd();"><img id="accessionssign" src="$gdb::util::webloc/web/minus.gif" alt=""> Accessions</span></div>};
		...
		{
			print
qq{<div class="mn2" style="border-top:1px solid #C3CCD3;"><span onclick="da('accessions');" onmouseover="mm(this,'accessions');" onmouseout="return nd();"><img id="accessionssign" src="$gdb::util::webloc/web/plus.gif" alt=""> Accessions</span></div>};
...
sub accessions
{
	...
	my $dbAccessionRec = gdb::oracle::dbAccessionsInfo($organism);
	my %dbAccession    = ();
	%dbAccession = %$dbAccessionRec;
...
qq{<td class="tdc"><input id="ckaccn$id" class="small" type="checkbox" name="ckaccn" value="$id" onclick="ckqry('$parms->{currquery}');" $checked $disabled></td>\n};
		...
		{
			$tmpline .=
qq{<img id="esign$id" src="$gdb::util::webloc/web/plus.gif" onclick="ckaccexp($id);" alt="" onmouseover="this.style.cursor='pointer';return overlib('click to display experiments');" onmouseout="return nd();"> };
		} else
		{
			$tmpline .= "&nbsp;&nbsp;&nbsp;&nbsp;";
		}
...
qq{<a href="http://www.ncbi.nlm.nih.gov/projects/geo/query/acc.cgi?acc=$gpl" target="_blank" onmouseover="return overlib('query this platform in Geo (new window)');" onmouseout="return nd();">$gpl</a>, };
	...
	my $accHead =
	  ( $col =~ /accession/ )
	  ? qq{ACCESSION<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">}
	  : "ACCESSION";
	my $expsHead =
	  ( $col =~ /exps/ )
	  ? qq{EXPS<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">}
	  : "EXPS";
	my $institutionHead =
	  ( $col =~ /institution/ )
	  ? qq{INSTITUTION_[PI]<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">}
	  : "INSTITUTION_[PI]";
	my $referenceHead =
	  ( $col =~ /reference/ )
	  ? qq{REFERENCE<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">}
	  : "REFERENCE";
	my $titleHead =
	  ( $col =~ /title/ )
	  ? qq{ACCESSION TITLE<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">}
	  : "ACCESSION TITLE";
	my $experimentHead =
	  ( $col =~ /experiment/ )
	  ? qq{EXPERIMENT DESIGN<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">}
	  : "EXPERIMENT DESIGN";
	my $arrayHead =
	  ( $col =~ /array/ )
	  ? qq{ARRAY DESIGN<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">}
	  : "ARRAY DESIGN";
	my $genomeHead =
	  ( $col =~ /genome/ )
	  ? qq{SAMPLE SOURCE<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">}
	  : "SAMPLE SOURCE";

	#czhang
	my $organismHead =
	  ( $col =~ /organism/ )
	  ? qq{ORGANISM<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">}
	  : "ORGANISM";

	#print "<pre>" . Dumper( $organismHead ) . "</pre>";
	my $platformHead =
	  ( $col =~ /platform/ )
	  ? qq{PLATFORM<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">}
	  : "PLATFORM";

	#czhang
	my $moduserHead =
	  ( $col =~ /moduser/ )
	  ? qq{MODUSER<img id="accessionssort" src="$gdb::util::webloc/web/$arrow.gif" alt="">}
	  : "MODUSER";
...
qq{<th><span class="shc" onclick="sm('moduser','$dir');" onmouseover="return overlib('Moduser, click to sort');" onmouseout="return nd();">$moduserHead</span></th>\n},
	...
	for my $i ( sort myColsort keys %line )
	{
		...
		print $line{$i}{exps},
		  ...
		  #czhang
		  $line{$i}{organism}, $line{$i}{platform},
...
sub accExperiments
{
	...
	my %acchm    = ();
	%acchm = %$acchmRef if $acchmRef;
	my ( $dbaccExpmRec, $expm_orderRef ) = gdb::oracle::dbgetAccExpm()
		...
		if ( $gdb::webUtil::frmData{id} == $dbaccExpm{$id}{expid} )
		{
			next
			  if ( $dbaccExpm{$id}{cntlgenome} !~
				   /$gdb::util::gnom{$parms->{genome}}{acc}/i );
			$ExpmCount++;
			if ( !$hasExp )
			{
				$hasExp = 1;
...
qq{<th class="thc" onmouseover="return overlib('Select/Unselect all');" onmouseout="return nd();"><input id="ckallexp$dbaccExpm{$id}{expid}" type="checkbox" name="ckallid" onclick="ckallexpmt(this,'ckaccn','ckexpm',$dbaccExpm{$id}{expid});ckqry('$parms->{currquery}');" $ckaccn></th>\n},
			...
			my $testgenome =
			  ...
			  ? $gdb::util::gnom{ $gdb::util::gnomacc{ $dbaccExpm{$id}
					  {testgenome} } }{sname}
			  : '';
			my $cntlgenome =
...
#czhang July 12, 2013
qq{<td class="tdc"><a onclick="nd();gm('downloadExpMetaData',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('Download Experiment MetaData');" onmouseout="return nd();">DownloadMetaData</a> &nbsp;&nbsp;</td>\n},
...
qq{<div class="hidden" id="arraydesign$gdb::webUtil::frmData{id}"></div>\n},
	  ...
	  #czhang scatter plot
	  qq{<div class="hidden" id="expplot$gdb::webUtil::frmData{id}"></div>\n},
...
sub providers
{
	my $dbexpProvidersRef =
	  gdb::oracle::dbexpProviders( $gdb::webUtil::frmData{id} );
...
{
	my $dbexpSummaryRef =
	  gdb::oracle::dbexpSummary( $gdb::webUtil::frmData{id} );
...
{
	my $dbarrayDesignRef =
	  gdb::oracle::dbarrayDesign( $gdb::webUtil::frmData{id} );
...
qq{<td class="tdc" bgcolor="#ebf0f2"><a onclick="window.open('$ENV{REQUEST_URI}?ajax=accinfo&accinfo=viewRawData&view=$dbsampInfo{$colsamp{$col}{$row}}{fname}&accession=$dbsampInfo{$colsamp{$col}{$row}}{accession}&sname=$dbsampInfo{$colsamp{$col}{$row}}{samid}&id=$dbsampInfo{$colsamp{$col}{$row}}{bioassays_id}', '_blank')" onmouseover="this.style.cursor='pointer';return overlib('View raw data (new window)');" onmouseout="return nd();">view</a></td>\n},
		...
		print qq{</tr>\n};
		print
qq{<tr><td colspan="$spancol"><div class="hidden" id="sampdetail$i$gdb::webUtil::frmData{id}"></div></td></tr>\n};
		$i++;
	}
...
sub sampdetail
{
	my ( $sampIDname, $dbsampDetailRef ) =
	  gdb::oracle::dbsampDetail( $gdb::webUtil::frmData{sampID} );
	my %dbsampDetail = %$dbsampDetailRef;
	my $dbsampDescriptionRef =
	  gdb::oracle::dbsampDescription( $gdb::webUtil::frmData{sampID} );
...
{
	my $opened = open( FILE,
"$gdb::util::datapath/$gdb::webUtil::frmData{accession}/$gdb::webUtil::frmData{view}"
	);
	print
	  qq{<html><head><title>$gdb::webUtil::frmData{sname} Raw Data</title>\n},
qq{<script type="text/javascript" src="/web/js/overlib/Mini/overlib_mini.js"></script><!-- overLIB (c) Erik Bosrup -->\n},
	...
	if ($opened)
	{
		...
		my ($labelRef) =
		  gdb::oracle::dbsampFilehead( $gdb::webUtil::frmData{id} );
		my %label = %$labelRef;
		print qq{<pre>\n},
...
sub expinfo
{
	...
	my ( $dbExpmRef, $expmorderRef ) =gdb::oracle::dbgetExpmInfo_VBI($expid);    #all experiments, needed for ouid
	
	my %dbExpm       = %$dbExpmRef;
	#my $expid        = $gdb::webUtil::frmData{id};
	my $dbexpInfoRef = gdb::oracle::dbexpInfo_VBI($expid);
...
qq{<tr><td class="tdc" colspan="3"><input class="ebtn" type="button" value="Update" onclick="updexperiment($expid,this.form)"></td><td class="small">$status</td></tr>\n}
	...
	print qq{<th class="thc">EXPERIMENT NAME</th>\n},
	  ...
	  #czhang added scatter
	  qq{<th class="thc">PLOT</th>\n},
	  ...
	  #czhang added Feb 11, 2013	
	  qq{<th class="thc">ORGANISM</th>\n},
	  qq{<th class="thc">STRAIN</th>\n},
	  qq{<th class="thc">MUTANT</th>\n},
	  qq{<th class="thc">CONDITION</th>\n},
	   qq{<th class="thc">SOURCE</th>\n};
...
#next if ($dbexpInfo{$id}{cntlgenome} !~ /$gdb::util::gnom{$parms->{genome}}{acc}/i);
		my $dbcolumnNameRef =
		  gdb::oracle::dbcolumnName( $dbexpInfo{$id}{samples} )
		...
		my $testgenome =
		  ...
		  ? $gdb::util::gnom{ $gdb::util::gnomacc{ $dbexpInfo{$id}
				  {testgenome} } }{sname}
		  : '';
		my $controlcolumn =
...
qq{<td class="tdl"><input class="tdl" type="text" size="80" maxlength="250" name="chgExpName$expid$expmID" value="$dbexpInfo{$id}{expname}"></td>\n};
		...
		#czhang scatter plot
		print
qq{<td class="tdc"><a onclick="expplot('scatter','$expmID','$expid');" onmouseover="this.style.cursor='pointer';return overlib('Scatter plot');" onmouseout="return nd();">Scatter</a></td>\n};
...
qq{<td class="tdc"><input class="tdr" type="text" size="5" maxlength="9" name="chgtimepoint$expid$expmID" value="$dbexpInfo{$id}{timepoint}"></td>\n};
		...
		print qq{<td class="tdc">$dbexpInfo{$id}{channels}</td>\n},
		  ...
		  qq{<td class="tdr">$dbexpInfo{$id}{organism}</td>\n},
		  qq{<td class="tdr">$dbexpInfo{$id}{strain}</td>\n},
		  qq{<td class="tdr">$dbexpInfo{$id}{mutant}</td>\n},
		  qq{<td class="tdr">$dbexpInfo{$id}{condition}</td>\n},
		  qq{<td class="tdr">$dbexpInfo{$id}{source}</td>\n};
	...
	for my $id ( sort { $a <=> $b } keys %dbexpInfo )
	{
		...
		} else
		{
			...
			#$change .= "timepoint=$newtp," 
			 $change .= "timepoint='$newtp'," #czhang changed at April 2, 2013
			  if $dbexpInfo{$id}{timepoint} ne $exptimept[$id];
			push @allchg,
"update pexp set $change moddate=sysdate,moduser='$gdb::webUtil::username' where id=$dbexpInfo{$id}{id}"
			  if $change;
		}
	}
	my $status;
...
{
	my $dbcuratedInfoRef =
	  gdb::oracle::dbcuratedInfo( $gdb::webUtil::frmData{id} );
	my %dbcuratedInfo  = %$dbcuratedInfoRef;
	my $dbexpDesignRef = gdb::oracle::dbexpDesign( $gdb::webUtil::frmData{id} );
	my %dbexpDesign    = %$dbexpDesignRef;
	my (
	...
	#initializing variables added by czhang
	if(!defined $pi){
		$pi="";
	}
	...
	if(!defined $growthcond){
		$growthcond="";
	}
	if(!defined $modification){
		$modification="";
	}
	if(!defined $arraydesign){
		$arraydesign="";
	}
	if(!defined $strain){
		$strain="";
	...
	if ($change)
	{
		my $rc = gdb::oracle::dbupdatecuratedInfo(
"update curated set $change moddate=sysdate,moduser='$gdb::webUtil::username' where expid=$gdb::webUtil::frmData{id}"
		);
		$status =
		  ($rc)
		  ? qq{<font color="red">* * * Error updating!! * * *</font>}
...
sub expdata
{
	...
	my $dbchannelCountsRef = gdb::oracle::dbchannelCounts_vbi($selid);
	...
	for my $i ( sort { $a <=> $b } keys %dbsampInfo )
	{
		...
		$numchannel = $gdb::webUtil::frmData{newchan}
		  if $gdb::webUtil::frmData{newchan};    #changed default channel
		if ( $numchannel == 1 )
		{
			$chgNumchan =
...
qq{<tr><td class="tdl"><b>PLOT INFORMATION</b> ($numchannel channel) $chgNumchan</td></tr>\n},
	  ...
	  #czhang Feb 20, 2013 
	 qq{<b>Data Process:</b> <input type="checkbox" id="log$selid" value="1">Log(base2) &nbsp; <input type="checkbox" id="normalize$selid" value="1">Normalize(loess) &nbsp; <input type="checkbox" id="antilog$selid" value="1">Anti-Log10 &nbsp; <input type="checkbox" id="antilog2$selid" value="1">Anti-Log2\n},	  	  
	...
	if (%c1data)
	{
		my @RMAfiles = <$gdb::util::datapath/$dbsampInfo{0}{accession}/*.RMA>
...
sub sel1Plot
{
	...
	my $dbsampInfoRef =
	  gdb::oracle::dbsampInfo( $gdb::webUtil::frmData{id} );
	my %dbsampInfo = %$dbsampInfoRef;
...
qq{<font color="red">No Platform available!  Please contact administrator.</font>};
	...
	my $normalize =
	  ( $gdb::webUtil::frmData{normalize} =~ /true/i ) ? 1 : 0;
	my $antilog = ( $gdb::webUtil::frmData{antilog} =~ /true/i ) ? 1 : 0;
	
	
	#CZHANG Feb 20, 2013
	my $antilog2 = ( $gdb::webUtil::frmData{antilog2} =~ /true/i ) ? 1 : 0;
	
	my $userma  = ( $gdb::webUtil::frmData{userma}  =~ /true/i ) ? 1 : 0;
	my $plottype =
	  ( $gdb::webUtil::frmData{plottype} )
	  ? $gdb::webUtil::frmData{plottype}
	  : '';
	my $datacol =
	  ( $gdb::webUtil::frmData{datacol} )
	  ? $gdb::webUtil::frmData{datacol} - 1
	...
	for my $bioassays_id ( sort keys %testsample )
	{
		...
		my $dbchannelSourceRef =
		  gdb::oracle::dbchannelSource($bioassays_id);
...
#antilog - convert log 10 to base number, log10(num)=val  antilog== 10^val = num
			...
			if ($antilog2)
			{
				#print "<pre> antilog2 is checked </pre>";
				#antilog - convert log2 to base number, log2(num)=val  antilog== 10^val = num
...
qq{<tr><td class="tdc" colspan="8"><font color="red">\tCounts are not equal!</font></td></tr>\n};
	...
	my ( $plotFile, $pmap ) =
	  gdb::plot::createExpPlot(
									$plottype,      $stddev,
									$cntlgenomeacc, \%plotTestdata,
									\%plotCntldata
	  );
	print
qq{<tr><td class="tdc" colspan="8"><img alt="" src="/tmpimage/$plotFile" border="1" usemap="#$gdb::webUtil::frmData{id}"></td></tr>},
	  qq{<map name="$gdb::webUtil::frmData{id}">$pmap</map>\n},
...
qq{<input class="ebtn" type="button" value="Download data" onclick="window.open('$gdb::util::urlpath/download.pl?type=plotdata&fname=$dataFilename&samp=Test($avgTestsname) / Control($avgCntlsname)','_blank')" onmouseover="this.style.cursor='pointer';return overlib('Download data');" onmouseout="return nd();">\n},
	...
	my ($vbi_organism,  $vbi_strain, $vbi_mutant, $vbi_cond, $vbi_timepoint);
	
	$vbi_organism=$vbi_strain=$vbi_mutant=$vbi_cond=$vbi_timepoint="";
		...
		print qq{<br>},
		  ...
		  qq{</tr>}, 
		     ...
		     #czhang
		   qq{<tr>}, 
		   qq{<td class="tdr"><b>Organism:</b></td>},
		   qq{<td> <input class="small" type="text" size="110" maxlength="250" id="vbi_organism$gdb::webUtil::frmData{id}" value="$vbi_organism"> </td>},
		   qq{</tr>}, 
		   
		   qq{<tr>}, 
		   qq{<td class="tdr"><b>Strain:</b></td>},
		   qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_strain$gdb::webUtil::frmData{id}" value="$vbi_strain"> </td>},
		   qq{</tr>}, 
		   
	       qq{<tr>}, 
		   qq{<td class="tdr"><b>Mutant:</b></td>},
		   qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_mutant$gdb::webUtil::frmData{id}" value="$vbi_mutant"> </td>},
		   qq{</tr>}, 
		   
		   qq{<tr>}, 
		   qq{<td class="tdr"><b>Condition:</b></td>},
		   qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_cond$gdb::webUtil::frmData{id}" value="$vbi_cond"> </td>},
		   qq{</tr>}, 
	
		    qq{<tr>},
		   qq{<td class="tdr"><b>Time Point:</b></td>},
		   qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_timepoint$gdb::webUtil::frmData{id}" value="$vbi_timepoint"></td>},
		   qq{</tr>}, 
		  ...
		  qq{<tr>}, qq{<td></td>\n},
          qq{<td><input class="ebtn" type="button" value="Save Experiment" onclick="savExptoDB_2(event,$gdb::webUtil::frmData{id},$gdb::webUtil::frmData{id})"></td>\n},
		  qq{</tr>}, 
...
sub SampleCorrelation
{
	...
	for my $samp1 ( sort keys %corrData )
	{
		...
		{
			my $corr = gdb::plot::stat_correlation( $corrData{$samp1},
													$corrData{$samp2} );
...
qq{<font color="red">No Platform available!  Please contact administrator.</font>};
	...
	my $plottype =
	  ...
	  : '';
	my $testcol =
	  ( $gdb::webUtil::frmData{testcol} )
	  ? $gdb::webUtil::frmData{testcol} - 1
	  : '';    #subtract 1 because file is 0-based
	my $testbkgd =
	  ( $gdb::webUtil::frmData{testbkgd} )
	  ? $gdb::webUtil::frmData{testbkgd} - 1
	  : '';
	my $cntlcol =
	  ( $gdb::webUtil::frmData{cntlcol} )
	  ? $gdb::webUtil::frmData{cntlcol} - 1
	  : '';
	my $cntlbkgd =
	  ( $gdb::webUtil::frmData{cntlbkgd} )
	  ? $gdb::webUtil::frmData{cntlbkgd} - 1
	  : '';
...
#		my $pcnt = scalar keys %pfids;
		...
		for my $i ( sort { $a <=> $b } keys %pfids )
		{
			...
			#czhang channel 2 doe snot need antilog2
			#if ($antilog2)
			#{
				#print "<pre> antilog2 is checked </pre>";
				#antilog - convert log2 to base number, log2(num)=val  antilog== 10^val = num
...
qq{<font color="red">ID refs are inconsistent between platform and soft xml !</font>\n};
		...
		#czhang
		my @trmArray = gdb::util::transformArray(@maxval);
...
qq{<tr><td class="tdc" colspan="8"><font color="red">\tCounts are not equal!</font></td></tr>\n};
		...
		{
			print qq{<font color="red">Problems with X and Y axis !</font>\n};
			return;
		}
		print
...
qq{<input class="ebtn" type="button" value="Download data" onclick="window.open('$gdb::util::urlpath/download.pl?type=plotdata&fname=$dataFilename&samp=$sample{$bioassays_id}{sampid}','_blank')" onmouseover="this.style.cursor='pointer';return overlib('Download data');" onmouseout="return nd();">\n},
		...
		if ( ( scalar keys %sample ) == 1
			 and $gdb::webUtil::useracclevel > 2 )
		{
...
sub writeDataToFile
{
	...
	my $geneLocRef =
	  gdb::oracle::dbgetGeneLoc($genomeacc);    #genome gene and start
...
{
	print
	  qq{<html><head><title>$gdb::webUtil::frmData{samp} Plot Data</title>\n},
	  qq{<link rel="stylesheet" type="text/css" href="/web/css/main.css">\n},
...
sub savExp
{
	...
	my ($expname, $vbi_organism,  $vbi_strain, $vbi_mutant, $vbi_cond, $vbi_timepoint);
	$expname=$vbi_organism=$vbi_strain=$vbi_mutant=$vbi_cond=$vbi_timepoint="";
...
qq{<td onmouseover="this.style.cursor='pointer';return overlib('Swap ExpName conditions. (Chan1 / Chan2)');" onmouseout="return nd();"><input class="ebtn" type="button" value="Swap Conditions" onclick="swapcond($inputid)"></td>},
	  qq{</tr>},
	    ...
	    #czhang
		   ...
		   qq{<td class="tdr"><b>Organism:</b></td>},
		   ##jgrissom_27Nov2012
		   qq{<td> <input class="small" type="text" size="110" maxlength="250" id="vbi_organism$inputid" value="$vbi_organism"> </td>},
#		   qq{<td> <input class="small" type="text" size="110" maxlength="250" id="vbi_organism$gdb::webUtil::frmData{id}" value="$vbi_organism"> </td>},
		   qq{</tr>}, 
		   
		   qq{<tr>}, 
		   qq{<td class="tdr"><b>Strain:</b></td>},
		   qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_strain$inputid" value="$vbi_strain"> </td>},
#		   qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_strain$gdb::webUtil::frmData{id}" value="$vbi_strain"> </td>},
		   qq{</tr>}, 
		   
	       qq{<tr>}, 
		   qq{<td class="tdr"><b>Mutant:</b></td>},
		   qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_mutant$inputid" value="$vbi_mutant"> </td>},
#		   qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_mutant$gdb::webUtil::frmData{id}" value="$vbi_mutant"> </td>},
		   qq{</tr>}, 
		   
		    qq{<tr>}, 
		   qq{<td class="tdr"><b>Condition:</b></td>},
		   qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_cond$inputid" value="$vbi_cond"> </td>},
#		   qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_cond$gdb::webUtil::frmData{id}" value="$vbi_cond"> </td>},
		   qq{</tr>}, 
	
		    qq{<tr>},
		   qq{<td class="tdr"><b>Time Point:</b></td>},
		   qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_timepoint$inputid" value="$vbi_timepoint"></td>},
#		   qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_timepoint$gdb::webUtil::frmData{id}" value="$vbi_timepoint"></td>},
		   qq{</tr>}, 
...
qq{<td class="tdl"><textarea class="small" id="info$inputid" rows="3" cols="110"></textarea></td>},
	  ...
	  qq{<tr>},
	  qq{<td></td>\n},
qq{<td><input class="ebtn" type="button" value="Save Experiment" onclick="savExptoDB_2(event,$selid,$inputid)"></td>\n},
	  qq{</tr>},
	  qq{</table>};
	$savExpInfo{dataFilename} = $dataFilename;
	gdb::webUtil::putSessVar( 'savExpInfo', \%savExpInfo );
...
sub savExptoDB
{
	...
	my $expname       = $gdb::webUtil::frmData{expname};
	my $vbi_organism  = $gdb::webUtil::frmData{vbi_organism};
	my $vbi_strain    = $gdb::webUtil::frmData{vbi_strain};
	my $vbi_cond      = $gdb::webUtil::frmData{vbi_cond};
	my $vbi_mutant    = $gdb::webUtil::frmData{vbi_mutant};
	my $vbi_timepoint = $gdb::webUtil::frmData{vbi_timepoint};
	if ( !$vbi_organism )
	{
		$vbi_organism = "";
	} 
	...
	#czhang vbi func
	my $exprows = gdb::oracle::checkExpDuplicate($savExpInfoRef);
		
	if ( $exprows > 0 )
	{
		print qq{<font color="red">* * * Experiment is already created * * *</font>};
	} else	{
		my $rc =
		  gdb::oracle::dbsaveExptoDB_vbi3(
								   $savExpInfoRef, $vbi_organism, $vbi_strain,
								   $vbi_mutant,    $vbi_cond,     $vbi_timepoint
		  );
		if ($rc)
		{
			print
			qq{<font color="green">* * * Experiment added successfull. ($rc) * * *</font>};
		} else
		{
			print
