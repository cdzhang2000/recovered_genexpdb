...
sub ajax {
	...
	if ( $gdb::webUtil::frmData{ginfo} ) {
		...
		gdb::geoupdate::geoUpdateMain()      if ( $gdb::webUtil::frmData{ginfo} =~ /^geoupdt/ );
			...
			#czhang July 12,2013
		gdb::info::downloadExpMetaData( ) if ( $gdb::webUtil::frmData{ginfo} =~ /^downloadExpMetaData/ );
	...
	if ( $gdb::webUtil::frmData{plot} ) {
		...
		#czhang added for scatter
		gdb::plot::expplot() if ( $gdb::webUtil::frmData{plot} =~ /^expplot/ );
	...
	if ( $gdb::webUtil::frmData{accinfo} ) {
		...
		#July 12, 2013
		gdb::accessions::downloadExpMetaData() if ( $gdb::webUtil::frmData{accinfo} =~ /^downloadExpMetaData/ );
	}
