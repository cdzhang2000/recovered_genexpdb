...
qq{<td valign="top"><a class="exmp" onclick="accinfo('expinfo',$gdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	...
	for my $id ( sort { $a <=> $b } keys %dbexpInfo ) {
		...
		#czhang scatter plot
        print qq{<td class="tdc"><a onclick="expplot('scatter','$expmID','$expid');" onmouseover="this.style.cursor='pointer';return overlib('Scatter plot');" onmouseout="return nd();">Scatter</a></td>\n};
...
#	my $cntlgenomeacc = $gdb::webUtil::frmData{cntlgenome};
	...
	#print "cntlgenomeacc= $gdb::webUtil::frmData{cntlgenome} <br>";
	...
	#czhang
	#while (my ($key, $value) = each(%dbplatformAnnot)){
    #print $key.", ".$value."<br />";
	#}
	...
	for my $bioassays_id ( sort keys %sample ) {
		...
		foreach my $line (@data) {
			...
			#print " id_ref = $id \t $id_ref <br>";
			
			#print " testcol = $testcol \t cntlcol = $cntlcol <br>";
			...
			#print " testVal := $cntlVal \t cntVal= $cntlVal <br>";
			...
			#print "pfids{$i}{ltag}    = $id_ref <br>";
			
			$pfids{$i}{testVal} = $testVal;
			
			#print "pfids{$i}{testVal} = $testVal <br>";
			
			$pfids{$i}{cntlVal} = $cntlVal;
			
			#print "pfids{$i}{cntlVal} = $cntlVal <br>";
		...
		for my $i ( sort { $a <=> $b } keys %pfids ) {	
			...
			if ( defined $cntldata{$id_ref} ) {
				...
				if ( $cntlVal ne ''  and looks_like_number($cntlVal) ){
					...
					#print " cntldata{$id_ref}{val} = $cntldata{$id_ref}{val} <br>";
				}
			} else {
...
#		  qq{<tr><th class="thc" colspan="8">$sample{$bioassays_id}{sampid} - $gdb::util::gnom{$gdb::util::gnomacc{$savExpInfo{cntlgenome}}}{sname}</th></tr>\n},
		...
		my ( $plotFile, $pmap ) = gdb::plot::createExpPlot( $plottype, $stddev, $cntlgenomeacc, \%plotTestdata, \%plotCntldata, $platform );
...
sub savExptoDB {
	...
	my $rc = gdb::oracle::dbsaveExptoDB_vbi2($savExpInfoRef);
	if ($rc) {
