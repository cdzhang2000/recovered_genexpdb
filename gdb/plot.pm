...
sub scatterPlot
{
	...
	my $hmcnt =
	  ( exists $gdb::webUtil::frmData{hmcnt} )
	  ? $gdb::webUtil::frmData{hmcnt}
	  : 0;
	my $expid = ( $gdb::webUtil::frmData{id} ) ? $gdb::webUtil::frmData{id} : 0;
	my $gene =
	  ( $gdb::webUtil::frmData{gene} ) ? $gdb::webUtil::frmData{gene} : '';
	my $selGene =
	  ( $gdb::webUtil::frmData{selGene} )
	  ? $gdb::webUtil::frmData{selGene}
	  : '';
	my $qryall =
	  ( $gdb::webUtil::frmData{qryall} ) ? $gdb::webUtil::frmData{qryall} : '';
	my $nsd =
	  ( $gdb::webUtil::frmData{nsd} =~ /undefined/ )
	  ? 2
	  : $gdb::webUtil::frmData{nsd};
	my ($dbExpmRecRef) =
	  gdb::oracle::dbgetExpmPlotInfo();    #get all experiment info
	...
	my $stdDev2 = $nsd * $std;
	my ( $plotFile, $pmap, $upregRef, $dnregRef, $datacnt ) =
	  createScatterPlot( $expid, $gene, $selGene, $std, $nsd, $platform );
	my %upreg = %$upregRef;
...
qq{<select class="small" onChange="replot('$expid',this.options[this.selectedIndex].value,document.mainFrm.nsd.value);" onmouseover="return overlib('Up regulated, sorted by ratio');" onmouseout="return nd();">\n},
	...
	{
		print
qq{<option value="$upreg{$ratio}{ltag}">$upreg{$ratio}{gene}</option>\n};
	}
	print
	  qq{</select>\n},
qq{&nbsp;&nbsp;&nbsp; <select class="small" onChange="replot('$expid',this.options[this.selectedIndex].value,document.mainFrm.nsd.value);" onmouseover="return overlib('Down regulated, sorted by ratio');" onmouseout="return nd();">\n},
	...
	{
		print
qq{<option value="$dnreg{$ratio}{ltag}">$dnreg{$ratio}{gene}</option>\n};
	}
	print
	  qq{</select>\n},
	  qq{&nbsp;&nbsp;&nbsp; Green line = StdDev(sum of ratio) ($std) * },
qq{<input class="nsd" type="text" size="3" maxlength="3" name="nsd" value="$nsd"> = $stdDev2 },
...
sub viewpdata
{
	...
	my $hmcnt =
	  ...
	  : 0;
	my $expid =
	  ( $gdb::webUtil::frmData{expid} ) ? $gdb::webUtil::frmData{expid} : 0;
...
sub linePlot
{
	...
	my ( $plotFile, $pmap, $numExps ) =
	  createLinePlot( $gdb::util::gnom{ $parms->{genome} }{acc},
					  $accessionID, $selGene, $qryall, $std );
	my $pos = ( $parms->{wrap} == 1 ) ? 'center' : 'left';
	if ($plotFile)
	{
		print
...
sub createExpPlot
{
	my ( $plottype, $stddev, $genomeacc, $plotTestdataRef, $plotCntldataRef,
		 $platform )
	  = @_;
 ...
 #my %geneLoc    = %$geneLocRef;
	my $geneLocRef =
	  gdb::oracle::dbgetGeneLocBasedPlatform($platform);  #genome gene and start
	my %geneLoc      = %$geneLocRef;
	...
	{
		my $testVal =
		  ( $plotTestdata{$id_ref} ne '' ) ? $plotTestdata{$id_ref} : '';
		my $cntlVal =
		  ( $plotCntldata{$id_ref} ne '' ) ? $plotCntldata{$id_ref} : '';
		if ( $plottype =~ /maplot|xyplot/
			 and ( $testVal eq '' or $cntlVal eq '' ) )
		{
		...
		} else
		{
			...
			} else
			{
				...
				$gLoc = $id_ref;
				$gLoc =~ s/[A-Za-z]|\(//g;
				$gLoc = substr( $gLoc, 0, 4 )
				  ;    #we may have multiple genes separated by ':'
			}
		}
		...
		#czhang
		#print "xdval = $xdval \n";
		if ( looks_like_number($xdval) )
		{    # $xdval is a numeric
			push @Xdata, $xdval;
			if ( $cntlVal ne '' )
			{
				if ( looks_like_number($cntlVal) )
				{
					push @Ydata, $cntlVal;
				} else
				{
	...
	#czhang $minY is initialized error
	if ( !defined($minY) )
	{
		$minY = 0;
		print "Min Y axis is not defined .\n";
	}
	if ( !defined($maxY) )
	{
		$maxY = 0;
		print "Max Y axis is not defined .\n";
	}
	...
	{
		$minX = 0;
		print "Min X axis is not defined .\n";
	}
	if ( !defined($maxX) )
	{
		$maxX = 0;
		print "Max X axis is not defined .\n";
	}
	if ( $maxX == $minX )
	{
		$maxX++;
	...
	my $step = ceil(
		 scalar(@ticks) / ( ( $y_size - $top_margin - $bottom_margin ) / 25 ) );
	for ( my $i = 0 ; $i < scalar(@ticks) ; $i += 1 )
	{
		...
		if ( !( $i % $step ) )
		{
			...
			$im->string( gdSmallFont, $left_margin - 3 - length($yst) * 6,
						 $yt - 7, $yst, $black );
		...
		$im->line( $left_margin + 1,
				   $yt, $x_size - $right_margin,
				   $yt, $grid_color );
	}
	##--X-axis
	@ticks = get_ticks( $minX, $maxX );
	$step = ceil(
		 scalar(@ticks) / ( ( $x_size - $left_margin - $right_margin ) / 70 ) );
	for ( my $i = 0 ; $i < scalar(@ticks) ; $i += 1 )
	{
		my $x = floor( $ticks[$i] );
		...
		if ( ( $i % $step ) == 0 )
		{
			$im->string( gdSmallFont,
						 $xt - ( length($x) * 6 / 2 ),
						 $y_size - $bottom_margin + 5,
						 $x, $black );
			$im->line( $xt, $y_size - $bottom_margin,
					   $xt, $y_size - $bottom_margin + 3, $black );
		} else
		{
			$im->line( $xt, $y_size - $bottom_margin,
					   $xt, $y_size - $bottom_margin + 1, $black );
		}
		$im->line( $xt, $top_margin, $xt, $y_size - $bottom_margin - 1,
				   $grid_color );
	}
	my ( $xlabel, $ylabel );
	##--Frame
	$im->line( $left_margin, $top_margin, $left_margin,
			   $y_size - $bottom_margin + 3, $black );
	$im->line( $left_margin - 3,
			   $y_size - $bottom_margin,
			   $x_size - $right_margin,
			   $y_size - $bottom_margin, $black );
	##--X-label
	$xlabel = 'Genome Location' if $plottype =~ /mbplot/;
	$xlabel = 'A'               if $plottype =~ /maplot/;
	$xlabel = 'X-Test'          if $plottype =~ /xyplot/;
	$im->string( gdSmallFont,
				 $x_size / 2 - ( length($xlabel) * 6 ) / 2,
				 $y_size - 20,
				 $xlabel, $black );
	##--Y-label
	...
	$im->stringUp( gdSmallFont, 5, $y_size / 2 + ( length($ylabel) * 6 ) / 2,
				   $ylabel, $black );
	##--Plot Data
	my $pmap = '';
	foreach my $ltag ( keys %plotData )
	{
		...
		my $gene =
		  ...
		  : $plotData{$ltag}{ltag};
		my $dmsg =
qq{Gene: <b>$gene</b> ($plotData{$ltag}{ltag})<br>Value: <b>$plotData{$ltag}{ratio}</b>};
		$im->arc( $xt, $yt, 2, 2, 0, 360, $blue );
		$pmap .=
qq{<area shape=circle coords="$xt,$yt,2" onMouseover="return overlib('$dmsg');" onMouseout="return nd();">};
	}
	##--STD lines
	if ( $stddev and $plottype !~ /xyplot/ )
	{
		...
		my $ystd =
		  $yoff +
		  $height -
		  ( ( $stddev * 1.0 - $minY ) / ( $maxY - $minY ) * $height );
		$im->line( $left_margin + 1,
				   $ystd, $x_size - $right_margin,
				   $ystd, $green );
...
#my ( $id, $gene, $selGene, $std, $nsd, $genomeacc ) = @_;
	my ( $id, $gene, $selGene, $std, $nsd, $platform ) = @_;

	#$id is the expmID here
	
	my $pmap = '';
#	my $genomeacc = '';

#	print "genome accession = $genomeacc";	
	#my $geneLocRef = testgdb::oracle::dbgetGeneLoc($genomeacc);	#genome gene and start
	
	##jgrissom_19Nov2012
	my $geneLocRef = gdb::oracle::dbgetGeneLocBasedPlatform($platform);	#genome gene and start

	my %geneLoc    = %$geneLocRef;
		
	#gene name space table
...
#			}
	...
	$plotData{$gLoc}{ratio} = $dbPlotData{$id}{ratio}
	  if ( $dbPlotData{$id}{ratio} );
	...
	if ( $dbPlotData{$id}{ratio}
		 and ( $dbPlotData{$id}{ratio} < ( $stdDev2 * -1.0 ) ) )
	{
...
my @ticks = get_ticks( $minY, $maxY );
my $step =
  ceil( scalar(@ticks) / ( ( $y_size - $top_margin - $bottom_margin ) / 25 ) );
for ( my $i = 0 ; $i < scalar(@ticks) ; $i += 1 )
{
	my $y = $ticks[$i];
	my $yt =
	  $yoff + $height - ( ( $y * 1.0 - $minY ) / ( $maxY - $minY ) * $height );
	my $yst;
	if ( !( $i % $step ) )
	{
		if ( $y == 0 )
...
@ticks = get_ticks( $minX, $maxX );
$step =
  ceil( scalar(@ticks) / ( ( $x_size - $left_margin - $right_margin ) / 70 ) );
for ( my $i = 0 ; $i < scalar(@ticks) ; $i += 1 )
{
	my $x = floor( $ticks[$i] );
	my $xt = $xoff + ( $x - $minX ) / ( $maxX - $minX ) * $width;
...
foreach my $tmp2 ( sort { $a <=> $b } keys %plotData )
{
	...
	my $gene =
	  ...
	  : $plotData{$tmp2}{ltag};
	my $dmsg =
qq{Gene: <b>$gene</b> ($plotData{$tmp2}{ltag})<br>Value: <b>$plotData{$tmp2}{ratio}</b>};
	if ( $selGene eq $plotData{$tmp2}{ltag} )
	{
		$im->filledEllipse( $xt, $yt, 10, 10, $red );
		$pmap .=
qq{<area shape=circle coords="$xt,$yt,8" onMouseover="return overlib('$dmsg');" onMouseout="return nd();">};
	} else
	{
		$im->arc( $xt, $yt, 2, 2, 0, 360, $blue );
		$pmap .=
qq{<area shape=circle coords="$xt,$yt,2" onMouseover="return overlib('$dmsg');" onMouseout="return nd();" onclick="replot('$id','$plotData{$tmp2}{ltag}',document.mainFrm.nsd.value);">};
	}
}
##--STD lines
if ($std)
{
	...
	my $ystd =
	  $yoff +
	  $height -
	  ( ( $stdDev2 * 1.0 - $minY ) / ( $maxY - $minY ) * $height );
	$im->line( $left_margin + 1, $ystd, $x_size - $right_margin, $ystd,
			   $green );
...
sub createLinePlot
{
	...
	foreach my $qry (@newVal)
	{
		...
		{
			( $geneRef, $ltagRef ) =
			  gdb::oracle::dbQryGenomeLtags( $qry, $genomeacc );
		}
		my @ltags = @$ltagRef;
	...
	my $dbPlotDataRef =
	  gdb::oracle::dbgetLPlotAllData($id);    #get all line plot data
	my %dbPlotData = %$dbPlotDataRef;
	my ( $dbaccExpmRec, $expm_orderRef ) =
	  gdb::oracle::dbgetAccExpm();            #get all accession experiments
	...
	for my $id (@expm_order)
	{
		...
		{
			push @Ydata, $dbPlotData{$id}{$ltag}{ratio}
			  if ( $dbPlotData{$id}{$ltag}{ratio} );
		}
	}
	my $numExps = @Xdata;
	if ( $numExps < 2 )
...
#red,blue,green,orange,skyBlue,violet,chocolate,darkBlue,darkMagenta,fuchsia,aquamarine,mediumPurple,purple,brown,cornflowerBlue
	...
	foreach (@geneNames)
	{
		...
		#set color index
		$geneLabel{$_} =
		  $im->colorAllocate( $labelColors[$i][0],
							  $labelColors[$i][1],
							  $labelColors[$i][2] );
		$i++;
		$i %= 15;
	...
	for my $id (@expm_order)
	{
		...
		{
			next
			  if ( !exists $dbPlotData{$id}{$ltag}{ratio}
				   || $dbPlotData{$id}{$ltag}{ratio} eq '' );
			my $gene =
			  ( $dbPlotData{$id}{$ltag}{gene} )
			  ? $dbPlotData{$id}{$ltag}{gene}
			  : $ltag;
			my $dmsg =
qq{OUID: <b>$ouid</b><br>Gene: <b>$gene</b> ($ltag)<br>Value: <b>$dbPlotData{$id}{$ltag}{ratio}</b>};
			$yt =
			  $yoff +
			  $height -
			  ( ( ( $dbPlotData{$id}{$ltag}{ratio} ) * 1.0 - $minY ) /
				( $maxY - $minY ) *
				$height );
			if (
				 abs( $dbPlotData{$id}{$ltag}{ratio} ) >=
				 ( 2 * $dbaccExpm{$id}{std} ) )
			{
				$pmap .=
qq{<area shape=circle coords="$xt,$yt,2" onMouseover="return overlib('$dmsg');" onMouseout="return nd();">};
			...
			if ( $sav_yt{$ltag} )
			{
				if ( $qryLtag{$ltag} )
				{
					...
					$selGeneLine{$ltag}{color} = $geneLabel{ $qryLtag{$ltag} }
					  if ( !defined $selGeneLine{$ltag}{color} );
...
qq{<area shape=circle coords="$xt,$yt,2" onMouseover="return overlib('$dmsg');" onMouseout="return nd();">};
		...
		{
			$im->line(
					   $selGeneLine{$ltag}{x1}, $selGeneLine{$ltag}{y1},
					   $selGeneLine{$ltag}{x2}, $selGeneLine{$ltag}{y2},
					   $selGeneLine{$ltag}{color}
			);
		}
	}
	my $xm = 0;
	...
	##-zero line
	my $ystd =
	  $yoff + $height - ( ( 0 - $minY ) / ( $maxY - $minY ) * $height );
	$im->line( $left_margin + 1, $ystd, $x_size - $right_margin, $ystd,
			   $black );
	my $plotFile = tmpFile();
	while ( -e $plotFile )
...
sub get_ticks
{
	...
	} else
	{
		$start =
		  floor( $min * 100 +
				 $even * 100 -
				 ( floor( $min * 100 ) % ( $even * 100 ) ) );
	}
	for ( $elem = $start, $i = 0 ;
		  $elem < $max * 100 ;
		  $elem += int( floor( $even * 100 ) ), $i++ )
	{
		$ticks[$i] = $elem / 100;
		if ( $i > 1000 )
		{
...
sub expplot
{
	my $parms = gdb::webUtil::getSessVar('parms');
	my $type =
	  ( $gdb::webUtil::frmData{type} ) ? $gdb::webUtil::frmData{type} : '';
	my $expmid =
	  ( $gdb::webUtil::frmData{expmid} ) ? $gdb::webUtil::frmData{expmid} : '';
	my $accessionid =
	  ( $gdb::webUtil::frmData{accnid} ) ? $gdb::webUtil::frmData{accnid} : '';
	my $nsd =
	  ( $gdb::webUtil::frmData{nsd} =~ /undefined/ )
	  ? 2
	  : $gdb::webUtil::frmData{nsd};
