...
{
	my $search_org = $_[0];
	print "Searched organism= $search_org";
...
sub dbsaveExptoDB_vbi3
{
   ...
   #  print "Timepoint=$timepoint \n";
	...
	$sql =
qq{ insert into pexp (id,eid,expname,expid,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,expstddev,platform,testgenome,cntlgenome,adddate,adduser, expmean, ORGANISM, STRAIN, MUTANT, CONDITION, TIMEPOINT) values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,SYSTIMESTAMP,?, ?, ?, ?, ?, ?, ?) };
	$sth = $dbh->prepare($sql);
	my $testcolumn = ( $savExpInfo{testcolumn} ) ? $savExpInfo{testcolumn} : '';
	my $testbkgd   = ( $savExpInfo{testbkgd} )   ? $savExpInfo{testbkgd}   : '';
	my $controlcolumn = '';
