...
sub dbMFunSel
{
	...
	$sql =
qq{ select mid, pid, mlevel, mfunction from sup_mfun_levels order by mid };
	$sth = $dbh->prepare($sql);
	$sth->execute();
	...
	my ($cnt);
	$sql =
qq{ select count(unique bnum) cnt from sup_mfun_bnum where blevel like ? || '%' };
	$sth = $dbh->prepare($sql);
	for my $mid (@midOrder)
	{
...
sub dbmfunQry
{
	...
	#query multifun
	$sql =
qq{ select unique substr(bnum,1,5) bnum from sup_mfun_bnum where REGEXP_LIKE(blevel,  '^' || ?) order by bnum};
	$sth = $dbh->prepare($sql);
	for my $mlevel (@mlevels)
	{
		$sth->execute($mlevel);
	...
	$sql =
qq{ select unique gene from genome where accession='NC_000913' and locus_tag = ?};
	$sth = $dbh->prepare($sql);
	for $bnum ( keys %mfun )
	{
		$mfunGene{$bnum} =
		  $bnum;    #use bnum as gene in case we do not find a gene
		$sth->execute($bnum);
...
sub dbpearsonCorr
{
	...
	my @newqry    = gdb::heatmap::ckQryLoc( $pcorrgene, $genomeacc )
	  ;    #we do this in case we get a number as a query?
	$pcorrgene = $newqry[0];
	my ( $geneRef, $ltagRef ) =
	  dbQryGenomeLtags( $pcorrgene, $genomeacc );    #pearson need a locusTag
	my @ltag      = @$ltagRef;
	my $pcorrltag = $ltag[0];
	...
	my $limitNum =
	  floor( ( $numExps * 75 ) / 100 )
	  ; #use 75% of the number of experiments selected as the Min number of Experiments cutoff
...
sub dbAccessionsInfo
{
	...
	if ($search_org)
	{
		...
		#print " dbAccessionsInfo =$search_org";
	} else
	{

		#print " dbAccessionsInfo is empty";
	}
...
#$sql =qq{ select a.id, a.eid, b.identifier, b.name, c.institution, c.pi, c.author, c.pmid, c.title, c.designtype, c.timeseries, c.treatment, c.growthcond, c.modification, c.arraydesign, c.strain, c.substrain, c.GEOMATCH organism, c.MODUSER moduser from experiment a, identifiable b, curated c where a.id=b.id(+) and (a.id=c.expid and c.status=3) order by to_number(substr(b.identifier,4)) };
	...
	{
		$sql =
qq{ select a.id, a.eid, b.identifier, b.name, c.institution, c.pi, c.author, c.pmid, c.title, c.designtype, c.timeseries, c.treatment, c.growthcond, c.modification, c.arraydesign, c.strain, c.substrain, c.GEOMATCH organism, c.MODUSER moduser from experiment a, identifiable b, curated c where a.id=b.id(+) and (a.id=c.expid and c.status=3) and c.GEOMATCH like '$search_org%' order by to_number(substr(b.identifier,4)) };
	} else
	{
		$sql =
qq{ select a.id, a.eid, b.identifier, b.name, c.institution, c.pi, c.author, c.pmid, c.title, c.designtype, c.timeseries, c.treatment, c.growthcond, c.modification, c.arraydesign, c.strain, c.substrain, c.GEOMATCH organism, c.MODUSER moduser from experiment a, identifiable b, curated c where a.id=b.id(+) and (a.id=c.expid and c.status=3) order by to_number(substr(b.identifier,4)) };
...
#add moduser
	...
	$sth->bind_columns(
						...
						\$name,       \$institution,  \$pi,
						\$author,     \$pmid,         \$title,
						\$designtype, \$timeseries,   \$treatment,
						\$growthcond, \$modification, \$arraydesign,
						\$strain,     \$substrain,    \$organism,
						\$moduser
	);
	while ( $row = $sth->fetchrow_arrayref )
	{
...
sub dbLabOrganization
{
	my %dbLabOrgRec;    #hash
	$sql =
qq{ select a.eid, b.identifier from organization a, identifiable b where a.id=b.id };
	$sth = $dbh->prepare($sql);
	$sth->execute();
	my ( $eid, $identifier );
...
sub dbExpmDesign
{
	...
	$sql =
qq{ select a.experiment_id ,c.value from experimentdesign a, types_experimentdesign b, ontologyentry c where a.id=b.experimentdesign_id and b.types_id=c.id };
	$sth = $dbh->prepare($sql);
	$sth->execute();
...
sub dbPlatformDesign
{
	...
	$dbh->{LongReadLen} = 2 * 1024 * 1024;    #2 meg
	$sql =
qq{ select a.eid, b.value, c.identifier, c.name from physicalarraydesign a, namevaluetype b, identifiable c where a.id=b.namevaluetype_id(+) and b.name='Manufacturer' and a.id=c.id };
	$sth = $dbh->prepare($sql);
	$sth->execute();
...
sub dbexpcntbyAcc
{
	my ($accnid) = @_;
	$sql =
qq{ select b.cntlgenome from curated a, pexp b where (a.expid = ? and a.status=3) and a.expid=b.expid };
	$sth = $dbh->prepare($sql);
	$sth->execute($accnid);
...
sub dbexpProviders
{
	...
	#organization
	$sql =
qq{ select unique c.identifier org from experiment a, organization b, identifiable c where a.id = ? and a.eid = b.eid and b.id = c.id };
	$sth = $dbh->prepare($sql);
	$sth->execute($id);
	my ( $org, @orgArr );
...
sub dbexpSummary
{
	...
	$dbh->{LongReadLen} = 2 * 1024 * 1024;    #2 meg
	$sql =
qq{ select name, value from namevaluetype where namevaluetype_id = ? order by id };
	$sth = $dbh->prepare($sql);
	$sth->execute($id);
	my ( $name, $value );
	my $i = 1;
	...
	#protocol
	$sql =
qq{ select b.text, c.identifier from experiment a, protocol b, identifiable c where a.id = ? and a.eid = b.eid and b.id = c.id order by b.id };
	$sth = $dbh->prepare($sql);
	$sth->execute($id);
	my ($identifier);
...
sub dbexpDesign
{
	my ($id) = @_;
	$sql =
qq{ select c.value from experimentdesign a, types_experimentdesign b, ontologyentry c where a.experiment_id = ? and a.id=b.experimentdesign_id and b.types_id=c.id  };
	$sth = $dbh->prepare($sql);
	$sth->execute($id);
	...
	$sth->finish;
	$dbexpDesign{Contributor_Desc} =
	  ( $dbexpDesign{Contributor_Desc} )
	  ? $dbexpDesign{Contributor_Desc}
	  : '';    #if exp has no recs from this query

	#curated table
	$sql =
qq{ select d.designtype, d.timeseries, d.treatment, d.growthcond, d.modification from curated d where d.expid = ? };
	$sth = $dbh->prepare($sql);
	$sth->execute($id);
	my ( $designtype, $timeseries, $treatment, $growthcond, $modification );
	$sth->bind_columns( \$designtype, \$timeseries, \$treatment,
						\$growthcond, \$modification );
	while ( $row = $sth->fetchrow_arrayref )
	{
		...
		$dbexpDesign{Genetic_Modifications} =
		  ($modification) ? $modification : '';
	}
	$sth->finish;
	return ( \%dbexpDesign );
...
qq{ select b.numberoffeatures, c.identifier, c.name, e.text, f.name vname, f.value from experiment a, arraydesign b, identifiable c, physicalarraydesign d, description e, namevaluetype f where a.id = ? and a.eid=b.eid and b.id=c.id and b.id=d.id and d.id= e.describable_id(+) and b.id=f.namevaluetype_id order by c.identifier };
	$sth = $dbh->prepare($sql);
	$sth->execute($id);
	my ( $numberoffeatures, $identifier, $name, $text, $vname, $value,
		 %dbarrayDesign );
	$sth->bind_columns( \$numberoffeatures, \$identifier, \$name, \$text,
						\$vname, \$value );
	while ( $row = $sth->fetchrow_arrayref )
	{
		$dbarrayDesign{$identifier}{platform} = ($name) ? $name : '';
		$dbarrayDesign{$identifier}{desc}     = ($text) ? $text : '';
		$dbarrayDesign{$identifier}{numf} =
...
q{ select a.bioassays_id, b.identifier accession, c.identifier samid, c.name samname, e.value fname, f.value gpl from bioassays_experiment a, identifiable b, identifiable c, extendable d, namevaluetype e, namevaluetype f where a.experiment_id = ? and a.experiment_id= b.id and a.bioassays_id=c.id and a.bioassays_id = d.label_id(+) and d.id=e.extendable_id(+) and (a.bioassays_id= f.namevaluetype_id and lower(f.name)='platform-ref') order by samname },
		undef, $id
	);
	foreach my $val (@$vals)
	{
		my ( $bioassays_id, $accession, $samid, $samname, $fname, $gpl ) =
		  @$val;
...
sub dbsampDescription
{
	...
	$dbh->{LongReadLen} = 2 * 1024 * 1024;    #2 meg
	$sql =
qq{ select a.name, a.value from namevaluetype a where a.namevaluetype_id = ? };
	$sth = $dbh->prepare($sql);
	$sth->execute($id);
...
sub dbsampFilehead
{
	...
	$dbh->{LongReadLen} = 2 * 1024 * 1024;    #2 meg
	$sql =
qq{ select a.value pos, lower(b.name) name, b.value from namevaluetype a,namevaluetype b, datasets c where a.namevaluetype_id = (select id from extendable where label_id = ? ) and a.id=b.namevaluetype_id(+) and a.eid=c.eid };
	$sth = $dbh->prepare($sql);
	$sth->execute($id);
...
{
	$sth = $dbh->prepare(
q{ select a.id from pexp a, curated b  where a.expid=b.expid and b.status=3 order by to_number(substr(a.accession,4)), a.exporder, a.timepoint, a.samples }
	);
	$sth->execute();
...
# return: hasfRef
#----------------------------------------------------------------------
sub dbgetExpmInfo_VBI
{
	my ($expid) = @_;
	$sql = qq{ select a.id from pexp a, curated b  where a.expid=b.expid and b.status=3 and a.expid=? order by to_number(substr(a.accession,4)), a.exporder, a.timepoint, a.samples };
	$sth = $dbh->prepare($sql);
	$sth->execute($expid);
...
#$sql =qq{ select id, expname, samples, timepoint, channels, testcolumn, testbkgd, controlcolumn, cntlbkgd, logarithm, normalize, antilog, userma, expstddev, exporder, platform, testgenome, cntlgenome, adduser, moduser,to_char(adddate, 'mm/dd/yy HH:MIam') as adate, to_char(moddate, 'mm/dd/yy HH:MIam') as mdate, expmean from pexp where expid = ? order by exporder, timepoint, samples };
	$sql =
qq{ select id, expname, samples, timepoint, channels, testcolumn, testbkgd, controlcolumn, cntlbkgd, logarithm, normalize, antilog, userma, expstddev, exporder, platform, testgenome, cntlgenome, adduser, moduser,to_char(adddate, 'mm/dd/yy HH24:MI') as adate, to_char(moddate, 'mm/dd/yy HH24:MI') as mdate, expmean from pexp where expid = ? order by adate, mdate, timepoint};
	$sth = $dbh->prepare($sql);
	$sth->execute($expid);
	my $i = 0;
	...
	my (
		 $id,         $expname,    $samples,   $timepoint,
		 $channels,   $testcolumn, $testbkgd,  $controlcolumn,
		 $cntlbkgd,   $logarithm,  $normalize, $antilog,
		 $userma,     $expstddev,  $exporder,  $platform,
		 $testgenome, $cntlgenome, $adduser,   $moduser,
		 $adate,      $mdate,      %dbexpInfo, $expmean
	);
	$sth->bind_columns(
						\$id,        \$expname,       \$samples,
						\$timepoint, \$channels,      \$testcolumn,
						\$testbkgd,  \$controlcolumn, \$cntlbkgd,
						\$logarithm, \$normalize,     \$antilog,
						\$userma,    \$expstddev,     \$exporder,
						\$platform,  \$testgenome,    \$cntlgenome,
						\$adduser,   \$moduser,       \$adate,
						\$mdate,     \$expmean
	);
	while ( $row = $sth->fetchrow_arrayref )
	{
		...
		$dbexpInfo{$i}{expstddev} =
		  ($expstddev) ? sprintf( "%05.3f", $expstddev ) : '';
...
#----------------------------------------------------------------------
# Accession Experiment Information from PEXP table
# input: string ID
# return: array ref
#----------------------------------------------------------------------
sub dbexpInfo_VBI
...
#$sql =qq{ select id, expname, samples, timepoint, channels, testcolumn, testbkgd, controlcolumn, cntlbkgd, logarithm, normalize, antilog, userma, expstddev, exporder, platform, testgenome, cntlgenome, adduser, moduser,to_char(adddate, 'mm/dd/yy HH:MIam') as adate, to_char(moddate, 'mm/dd/yy HH:MIam') as mdate, expmean from pexp where expid = ? order by exporder, timepoint, samples };
#czhang order only by time point
	$sql =
qq{ select id, expname, samples, timepoint, channels, testcolumn, testbkgd, controlcolumn, cntlbkgd, logarithm, normalize, antilog, userma, expstddev, exporder, platform, testgenome, cntlgenome, adduser, moduser,to_char(adddate, 'mm/dd/yy HH24:MI') as adate, to_char(moddate, 'mm/dd/yy HH24:MI') as mdate, expmean, organism, strain, mutant, condition, source from pexp where expid = ? order by adate};
	
	$sth = $dbh->prepare($sql);
	$sth->execute($expid);
	my $i = 0;

	#czhang added my $expmean and column with $expmean, organism, strain, mutant, condition, source
	my (
		 $id,         $expname,    $samples,   $timepoint,
		 $channels,   $testcolumn, $testbkgd,  $controlcolumn,
		 $cntlbkgd,   $logarithm,  $normalize, $antilog,
		 ...
		 $testgenome, $cntlgenome, $adduser,   $moduser,
		 $adate,      $mdate,      %dbexpInfo, $expmean,
		 $organism,   $strain,     $mutant,    $condition, $source
	);
	...
	while ( $row = $sth->fetchrow_arrayref )
	{
		...
		#czhang added exp mean
		$dbexpInfo{$i}{expmean} = ($expmean) ? $expmean : '';
		$dbexpInfo{$i}{organism} = ($organism) ? $organism : '';
		$dbexpInfo{$i}{strain} = ($strain) ? $strain : '';
		$dbexpInfo{$i}{mutant} = ($mutant) ? $mutant : '';
		$dbexpInfo{$i}{condition} = ($condition) ? $condition : '';
		$dbexpInfo{$i}{source} = ($source) ? $source : '';
...
{
	my ($sample) = @_;
	my @sampArr = split( /\,|\//, $sample )
	  ; #we may have one or more sample, but we only need the first one to look up the column name
	$dbh->{LongReadLen} = 2 * 1024 * 1024;    #2 meg
	$sql =
qq{ select c.value pos, d.value name from identifiable a, extendable b, namevaluetype c, namevaluetype d where a.identifier = ? and a.id = b.label_id and b.id = c.namevaluetype_id and c.id = d.namevaluetype_id and d.name = 'Name' };
	...
	{
		my $sampPos = $pos
		  ; #column number entered when experiment created started with 1(instead of 0)
		--$sampPos;
...
sub dbdlexpData
{
	...
	$sql =
qq{ select pavg, pratio, locustag from pdata where pexp_id=? order by locustag};
	$sth = $dbh->prepare($sql);
	$sth->execute($pexp_id);
...
sub dbcuratedInfo
{
	my ($id) = @_;
	$sql =
qq{ select b.name dbtitle, a.expid,a.accession,a.status,a.institution,a.pi,a.author,a.pmid,a.title,a.designtype,a.timeseries,
	...
	my %curatedInfo;
	my (
		 $dbtitle,     $expid,        $accession,   $status,
		 $institution, $pi,           $author,      $pmid,
		 $title,       $designtype,   $timeseries,  $treatment,
		 $growthcond,  $modification, $arraydesign, $strain,
		 $substrain,   $info,         $adduser,     $moduser,
		 $adate,       $mdate
	);
	$sth->bind_columns(
						\$dbtitle,    \$expid,        \$accession,
						\$status,     \$institution,  \$pi,
						\$author,     \$pmid,         \$title,
						\$designtype, \$timeseries,   \$treatment,
						\$growthcond, \$modification, \$arraydesign,
						\$strain,     \$substrain,    \$info,
						\$adduser,    \$moduser,      \$adate,
						\$mdate
	);
...
sub dbchannelCounts
{
	$dbh->{LongReadLen} = 2 * 1024 * 1024;    #2 meg
	$sql =
qq{ select value, namevaluetype_id from namevaluetype where name='Channel-Count' };
	$sth = $dbh->prepare($sql);
	$sth->execute();
...
# return: hash
#----------------------------------------------------------------------
sub dbchannelCounts_vbi
{
	my ($experimentID) = @_;
	$dbh->{LongReadLen} = 2 * 1024 * 1024;    #2 meg
	$sql =
qq{ select value, namevaluetype_id from bioassays_experiment a, namevaluetype b where a.bioassays_id = b.namevaluetype_id and b.name='Channel-Count' and a.experiment_id=? };
	$sth = $dbh->prepare($sql);
...
sub dbgenomeLtags
{
	...
	my %genomeLtags;
	my $vals = $dbh->selectall_arrayref(
q{ select unique locus_tag from genome where accession=? and locus_tag is not null },
		undef, $genomeacc
	);
	foreach my $val (@$vals)
	{
...
sub dbchannelSource
{
	...
	my @dbchannelSource;
	my $vals = $dbh->selectall_arrayref(
q{ select b.value from channels_bioassay a, namevaluetype b where a.bioassay_id = ? and a.channels_id = b.namevaluetype_id and name = 'Source' order by b.id },
		undef, $id
	);
	foreach my $val (@$vals)
	{
...
qq{ select a.id, a.expid, a.accession, a.expname, a.expstddev, a.platform, b.name, c.title from pexp a, identifiable b, curated c where a.expid=b.id and a.expid= c.expid order by to_number(substr(a.accession,4)), a.exporder, a.timepoint, a.samples };
	...
	$sth->execute();
	my %dbExpmRec;    #hash
	my ( $id, $expid, $accession, $expname, $expstddev, $platform, $name,
		 $title );
	my $i = 1;
	$sth->bind_columns( \$id, \$expid, \$accession, \$expname, \$expstddev,
						\$platform, \$name, \$title );
	while ( $row = $sth->fetchrow_arrayref )
	{
...
sub dbgetSPlotData
{
	...
	$sql =
qq{ select pratio, locustag from pdata where pexp_id=? and pratio is not null };
	$sth = $dbh->prepare($sql);
...
{
	$sql =
qq{ select unique b.name, b.ltag from ns_data a, ns_data b  where  a.ltag=b.ltag and b.type='gene' };
	$sth = $dbh->prepare($sql);
	$sth->execute();
...
sub dbgetGene
{
	my ( $ltag, $genome ) = @_;
	$sql =
qq{ select unique b.name from ns_data a, ns_data b  where lower(a.name)=lower(?) and a.ltag=b.ltag and b.type='gene' and b.genome=? };
	$sth = $dbh->prepare($sql);
...
sub dbgetGeneLoc
{
	my ($genome) = @_;
	$sql =
qq{ select unique locus_tag, gene, sstart from genome where accession=? and locus_tag is not null order by to_number(sstart)};
	$sth = $dbh->prepare($sql);
	$sth->execute($genome);
...
# return: ref to genome location
#----------------------------------------------------------------------
sub dbgetGeneLocBasedPlatform
{
	my ($platform) = @_;
	$sql =
qq{ select unique g.locus_tag, gene, sstart from genome g, PLATFORMANNOT p where PLATFORM=? and p.locustag=g.locus_tag};
	$sth = $dbh->prepare($sql);
	$sth->execute($platform);
	my ( $locus_tag, $gene, $sstart, %geneLoc );
	$sth->bind_columns( \$locus_tag, \$gene, \$sstart );
	my $i = 1;
	while ( $row = $sth->fetchrow_arrayref )
...
# return: ref to genome location
#----------------------------------------------------------------------
sub dbgetGeneStartLoc
{
	my ($genome) = @_;
	$sql =
qq{ select unique locus_tag, sstart from genome where accession=? and locus_tag is not null order by to_number(sstart)};
	$sth = $dbh->prepare($sql);
	$sth->execute($genome);
	my ( $locus_tag, $sstart, %geneStartLoc );
	$sth->bind_columns( \$locus_tag, \$sstart );
	while ( $row = $sth->fetchrow_arrayref )
	{
		$geneStartLoc{$locus_tag} = $sstart;
	}
	$sth->finish;
	return \%geneStartLoc;
}
...
sub dbgetNSgeneLoc
{
	my ($genome) = @_;
	$sql =
qq{ select unique ltag, name from ns_data where genome=? and type in ('locustag','old_locus_tag') };
	$sth = $dbh->prepare($sql);
	$sth->execute($genome);
...
sub dbgetNSinfo
{
	...
	#query name
	$sql =
	  qq{ select ltag, genome, type from ns_data where lower(name)=lower(?) };
	$sth = $dbh->prepare($sql);
	$sth->execute($query);
...
sub dbQueryNamespace
{
	my ($query) = @_;
	$sql =
qq{ select nstype, nsid from k12namespace where lower(REGEXP_REPLACE(namespace, '\\(obsolete\\)', '')) = lower(?) };
	$sth = $dbh->prepare($sql);
	$sth->execute($query);
	...
	@searchID = @other
	  if !@searchID
	;   #if we do not have a NC_000913:gene or NC_000913:locusTag then use other
	my ( @genes, @ltags );
	if (@searchID)
	{
...
sub dbgetLPlotAllData
{
	...
	my %nsGeneLtag    = %$nsGeneLtagRef;
	$sql =
qq{ select a.id, b.pratio, b.locustag from pexp a, pdata b where a.expid=? and a.id=b.pexp_id and b.pratio is not null };
	$sth = $dbh->prepare($sql);
...
sub dbgetAccExpm
{
	...
	$sql =
qq{ select a.id, a.expid, a.expname, a.channels, a.timepoint, a.expstddev, a.platform, a.testgenome, a.cntlgenome from pexp a order by to_number(substr(a.accession,4)), a.exporder, a.timepoint, a.samples };
	$sth = $dbh->prepare($sql);
	$sth->execute();
	my %dbaccExpmRec;    #hash
	my (
		 $id,        $ouid,      $expid,    $expname,    $channels,
		 $timepoint, $expstddev, $platform, $testgenome, $cntlgenome
	);
	my @expm_order;
	$sth->bind_columns(
						\$id,       \$expid,      \$expname,
						\$channels, \$timepoint,  \$expstddev,
						\$platform, \$testgenome, \$cntlgenome
	);
...
sub dbgetDownloadInfo
{
   ...
   #$sql =qq{ select a.id, a.expid, a.accession, a.expname, a.expstddev, a.samples, a.timepoint, a.channels, a.testcolumn, a.testbkgd, a.controlcolumn, a.cntlbkgd, a.logarithm, a.normalize, a.userma, b.name, c.title from pexp a, identifiable b, curated c where a.expid=b.id and a.expid= c.expid order by to_number(substr(a.accession,4)), a.exporder, a.timepoint, a.samples };
	
	$sql =qq{ select a.id, a.expid, a.accession, a.expname, a.expstddev, a.samples, a.timepoint, a.channels, a.testcolumn, a.testbkgd, a.controlcolumn, a.cntlbkgd, a.logarithm, a.normalize, a.userma, b.name, c.title, a.organism, a.strain, a.mutant, a.condition, a.source from pexp a, identifiable b, curated c where a.expid=b.id and a.expid= c.expid order by to_number(substr(a.accession,4)), a.exporder, a.timepoint, a.samples };
	
	$sth = $dbh->prepare($sql);
	$sth->execute();
	my $i = 0;
	my (
		 $id,         $expid,     $accession,     $expname,
		 $expstddev,  $samples,   $timepoint,     $channels,
		 $testcolumn, $testbkgd,  $controlcolumn, $cntlbkgd,
		 $logarithm,  $normalize, $userma,        $name,
		 $title,	  $organism,  $strain,        $mutant,
		 $condition,  $source,  
		 %dbExpmRec
	);
	$sth->bind_columns(
						\$id,        \$expid,         \$accession,
						\$expname,   \$expstddev,     \$samples,
						\$timepoint, \$channels,      \$testcolumn,
						\$testbkgd,  \$controlcolumn, \$cntlbkgd,
						\$logarithm, \$normalize,     \$userma,
						\$name,      \$title,         \$organism,
						\$strain,    \$mutant,         \$condition,  \$source						
	);
	...
	while ( $row = $sth->fetchrow_arrayref )
	{
		...
		$dbExpmRec{$i}{organism}     = ($organism)     ? $organism    : '';
		$dbExpmRec{$i}{strain}       = ($strain)       ? $strain      : '';
		$dbExpmRec{$i}{mutant}       = ($mutant)       ? $mutant      : '';
		$dbExpmRec{$i}{condition}    = ($condition)    ? $condition   : '';
		$dbExpmRec{$i}{source}       = ($source)       ? $source      : '';
...
#----------------------------------------------------------------------
# get next Pexp sequence number
# input: none
# return: hasfRef
#----------------------------------------------------------------------
sub dbgetPexpNextSeq
{
	$sql = qq{ select pexp_id_seq.nextval seqID from dual };
	$sth = $dbh->prepare($sql);
	$sth->execute();
	my $seqID = 0;
	$sth->bind_columns( \$seqID );
...
#CZHANG added exp mean value
	$sql =
qq{ insert into pexp (id,eid,expname,expid,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,expstddev,platform,testgenome,cntlgenome,adddate,adduser, expmean) values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,sysdate,?, ?) };
	$sth = $dbh->prepare($sql);
	my $testcolumn = ( $savExpInfo{testcolumn} ) ? $savExpInfo{testcolumn} : '';
	my $testbkgd   = ( $savExpInfo{testbkgd} )   ? $savExpInfo{testbkgd}   : '';
	my $controlcolumn = '';
	$controlcolumn = ( $savExpInfo{datacol} ) ? $savExpInfo{datacol} : ''
	  if ( $savExpInfo{channels} =~ /1/ );
	$controlcolumn =
	  ( $savExpInfo{controlcolumn} ) ? $savExpInfo{controlcolumn} : ''
	  if ( $savExpInfo{channels} =~ /2/ );
	...
	if (
		 !$sth->execute(
						 $pexp_id,
						 $expEID,
						 $gdb::webUtil::frmData{expname},
						 $savExpInfo{expid},
						 $savExpInfo{accession},
						 $savExpInfo{samples},
						 $savExpInfo{channels},
						 $testcolumn,
						 $testbkgd,
						 ...
						 $userma,
						 $savExpInfo{plottype},
						 $gdb::webUtil::frmData{info},
						 $savExpInfo{expstddev},
						 $savExpInfo{platform},
						 $savExpInfo{testgenome},
						 $savExpInfo{cntlgenome},
						 $gdb::webUtil::username,
						 $savExpInfo{expmean}
		 )
	  )
	{
	...
	#save data to pdata
	my $sqldata =
qq{ insert into pdata (id, eid, pexp_id, pavg, pratio, locustag, multid ) values ( ?, ?, ?, ?, ?, ?, ? ) };
	...
	foreach my $line (@data)
	{
		...
		foreach my $lt (@ltags)
		{
			...
			{
				$parent = $pdata_id
				  ; #if we only have 1 ltag then id=multid, if multiple ltags then multid = parentID
			}
			if (
				 !$sthdata->execute( $pdata_id, $expEID, $pexp_id, $test,
									 $cntl,     $lt,     $parent )
			  )
			{
...
#CZHANG added exp mean value
	$sql =
qq{ insert into pexp (id,eid,expname,expid,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,expstddev,platform,testgenome,cntlgenome,adddate,adduser, expmean) values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,SYSTIMESTAMP,?, ?) };
	$sth = $dbh->prepare($sql);
	my $testcolumn = ( $savExpInfo{testcolumn} ) ? $savExpInfo{testcolumn} : '';
	my $testbkgd   = ( $savExpInfo{testbkgd} )   ? $savExpInfo{testbkgd}   : '';
	my $controlcolumn = '';
	...
	#print "Time before open file is: $timenow\n";
	#get data file
	my $opened = open( FILE, "/run/shm/$savExpInfo{dataFilename}" );
	if ( !$opened )
	{
...
qq{ insert into pdata (id, eid, pexp_id, pavg, pratio, locustag, multid ) values ( ?, ?, ?, ?, ?, ?, ? ) };
	my $sthdata = $dbh->prepare($sqldata);
	my ( @pid, @eid, @pexpid, @pavg, @ratio, @locustag, @multid );

	#czhang print system time
	#$timenow = localtime(time);
	#print "Time begin insert row into pdata: $timenow\n";
	foreach my $line (@data)
	{
		chop($line);
		my ( $gene, $ltag, $test, $cntl ) = split( /\t/, $line );
	...
	my $tuples =
	  $sthdata->execute_array( { ArrayTupleStatus => \my @tuple_status },
				\@pid, \@eid, \@pexpid, \@pavg, \@ratio, \@locustag, \@multid );
	$sthdata->finish;
	if ($tuples)
	{
	...
	{
		for my $tuple ( 0 .. @multid - 1 )
		{
			my $status = $tuple_status[$tuple];
			$status = [ 0, "Skipped" ] unless defined $status;
			next unless ref $status;
			printf "Failed to insert (%s,%s, %s,): %s\n",
			  $eid[$tuple], $pexpid[$tuple], $locustag[$tuple],
			  $status->[1];
		}
	}
	...
	#czhang print system time
	#$timenow = localtime(time);
	#print "Time end insert row into padata: $timenow\n";
	#$dbh->commit();
	return $pexp_id;
}
...
# input: array
# return: int
# added foreign key pexp_id references id of pexp
# sqldr loader can load some records and reject rows having any problem e.g., data format error
# commit only when all rows are loaded without rejection
#----------------------------------------------------------------------
sub dbsaveExptoDB_vbi2
{
	my ($savExpInfoRef) = @_;
	my %savExpInfo      = %$savExpInfoRef;
	my $pexp_id         = dbgetPdataNextSeq();
...
qq{ insert into pexp (id,eid,expname,expid,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,expstddev,platform,testgenome,cntlgenome,adddate,adduser, expmean) values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,SYSTIMESTAMP,?, ?) };
	...
	} else
	{
		...
		close(FILE);
		shift(@data);    #remove heading line
		open MYFILE, ">/var/www/modperl/gdb/pdata/pdata.txt"
		...
		close MYFILE;
		my $DB =
"genexpdb/vb1g3n3xpdb\@genexpdb-restore.ccrlikknzibd.us-east-1.rds.amazonaws.com:3306/GENEXPDB";
		my $pdata_ctl = "/var/www/modperl/gdb/pdata/pdata.ctl";
		my $pdata_dat = "/var/www/modperl/gdb/pdata/pdata.txt";
		my $pdata_log = "/var/www/modperl/gdb/pdata/pdata.log";
		my $pdata_bad = "/var/www/modperl/gdb/pdata/pdata.bad";
		my $cmd       = "/home/ubuntu/oracle/product/db11gR2/bin/sqlldr";
		my $insertpdata = system(
"$cmd $DB control=$pdata_ctl data=$pdata_dat log=$pdata_log bad=$pdata_bad"
		);

		if ( $insertpdata != 0 )
		{
			print "Cannot insert pdata table.<br>";
			$dbh->rollback;
			return 0;
		} else
		{
			$sth->finish;
			$dbh->commit;
			print "updated pexp and pdata table.<br>";
			return $pexp_id;
		}
	}
}
...
# commit only when all rows are loaded without rejection
#----------------------------------------------------------------------
sub dbsaveExptoDB_vbi3
{
	my ($savExpInfoRef, $organism, $strain, $mutant, $condition, $timepoint) = @_;
	my %savExpInfo      = %$savExpInfoRef;
	my $pexp_id         = dbgetPdataNextSeq();
	my $expEID          = dbgetExpEID( $savExpInfo{expid} );
	if ( !$pexp_id or !$expEID )
	{
	 ...
	 if(! $organism){
     	...
     	if(length($organism)>=200){
     		$organism=substr($organism, 1, 100);
     	}
     #print "organism=$organism \n";
      ...
      if(! $strain){
    	...
    	if(length($strain)>=200){
     		$strain=substr($strain, 1, 100);
     	}
     }
     if(! $mutant){
     	$mutant="";
     }else{
     	if(length($mutant)>=200){
     		$mutant=substr($mutant, 1, 100);
     	}
     }
     if(! $condition){
     	$condition="";
   		...
   		if(length($condition)>=200){
     		$condition=substr($condition, 1, 100);
     	}
     }
     if(! $timepoint){
     	$timepoint="";
     }else {
 		if(length($timepoint)>=200){
     		$timepoint=substr($timepoint, 1, 100);
     	}
     }          
	
	
	$sql =
qq{ insert into pexp (id,eid,expname,expid,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,expstddev,platform,testgenome,cntlgenome,adddate,adduser, expmean, ORGANISM, STRAIN, MUTANT, CONDITION, TIMEPOINT, antilog2) values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,SYSTIMESTAMP,?, ?, ?, ?, ?, ?, ?, ?) };
	$sth = $dbh->prepare($sql);
	my $testcolumn = ( $savExpInfo{testcolumn} ) ? $savExpInfo{testcolumn} : '';
	my $testbkgd   = ( $savExpInfo{testbkgd} )   ? $savExpInfo{testbkgd}   : '';
	my $controlcolumn = '';
	...
	#czhang Feb 20, 2013
	my $antilog2  = ( $savExpInfo{antilog2} )  ? $savExpInfo{antilog2}    : 0;
	...
	if ( !$insertpexp )
	{
		print "Cannot insert experiment data. It may be duplicated sample<br>";
		return 0;
	} else
	{
		...
		foreach my $line (@data)
		{
			...
			foreach my $lt (@ltags)
			{
				...
				$lt =~ s/\s+$//;
				$tempString =
				    $expEID . "\t"  # Pdata column EID , PEXP tabl column : EID
				  . $pexp_id . "\t" # PData column PEXP_ID, PEXP table column ID
				  . $test . "\t" 
				  . $cntl . "\t"
				  . $lt . "\n";
				print MYFILE $tempString;
...
#check if the experiment is already created based on the expid and samples  of pexp table

sub checkExpDuplicate
{
	my $savExpInfoRef = $_[0];
	my %savExpInfo    = %$savExpInfoRef;

	#check if the experiment is already created
	my $expid   = $savExpInfo{expid};
	my $samples = $savExpInfo{samples};
	my $sql =
"select count(*) from pexp where expid=\'$expid\' and samples=\'$samples\'";
	$sth = $dbh->prepare($sql);
	$sth->execute();
	my $exprows = 0;
...
qq{ select gene,locus_tag,feature,sstart,sstop,orientation,join,gi,swissprot,ecogene,ecocyc,geneid,asap,synonyms,ec_number,function,note,old_locus_tag,product,protein_id from genome where (accession=?) and (lower(locus_tag)=lower(?)) };
	...
	my (
		 $gene,      $locus_tag,     $feature,   $sstart,
		 $sstop,     $orientation,   $join,      $gi,
		 $swissprot, $ecogene,       $ecocyc,    $geneid,
		 $asap,      $synonyms,      $ec_number, $function,
		 $note,      $old_locus_tag, $product,   $protein_id,
		 %dbannot
	);
	$sth->bind_columns(
						\$gene,     \$locus_tag, \$feature,
						\$sstart,   \$sstop,     \$orientation,
						\$join,     \$gi,        \$swissprot,
						\$ecogene,  \$ecocyc,    \$geneid,
						\$asap,     \$synonyms,  \$ec_number,
						\$function, \$note,      \$old_locus_tag,
						\$product,  \$protein_id
	);
	while ( $row = $sth->fetchrow_arrayref )
	{
		...
		$dbannot{orientation} =
		  checkDups( $orientation, $dbannot{orientation} );
		...
		$dbannot{old_locus_tag} =
		  checkDups( $old_locus_tag, $dbannot{old_locus_tag} );
...
sub dbQryGenomeLtags
{
	...
	$sql =
qq{ select unique gene, locus_tag from genome where (accession=?) and (lower(gene)=lower(?) or lower(locus_tag)=lower(?)) };
	$sth = $dbh->prepare($sql);
...
sub dbgetAnnotStartStop
{
	my ($genomeacc) = @_;
	$sql =
qq{ select sstart, sstop from genome where accession=? and feature='source' and sstart='1' };
	$sth = $dbh->prepare($sql);
...
sub dbgetStartStop
{
	my ( $query, $genomeacc ) = @_;
	$sql =
qq{ select unique sstart, sstop from genome where (accession=?) and (lower(gene)=lower(?) or lower(locus_tag)=lower(?)) };
	$sth = $dbh->prepare($sql);
	$sth->execute( $genomeacc, $query, $query );
...
{
	$sth = $dbh->prepare(
q{ select a.id, a.expid, a.accession, a.expname, a.expstddev, a.cntlgenome, b.name, c.title from pexp a, identifiable b, curated c where a.expid=b.id and a.expid= c.expid and c.status=3 order by to_number(substr(a.accession,4)), a.exporder, a.timepoint, a.samples }
	);
	$sth->execute();
	my (%dbexpInfo);
	my ( $id, $expid, $accession, $expname, $expstddev, $cntlgenome, $name,
		 $title );
	my $i = 1;
	$sth->bind_columns( \$id, \$expid, \$accession, \$expname, \$expstddev,
						\$cntlgenome, \$name, \$title );
	while ( $row = $sth->fetchrow_arrayref )
	{
		$dbexpInfo{$i}{id}         = $id;
...
sub dbgetheatmapData
{
	...
	$sth = $dbh->prepare(
q{ select pexp_id, pratio, locustag from pdata where lower(locustag) in ( select * from THE ( select cast( str2tbl( lower(?) ) as mytableType ) from dual ) ) }
	);
	...
	while ( $row = $sth->fetchrow_arrayref )
	{
		...
		if ( exists $dbHmData{$pexp_id}{ratio} )
		{
			...
			{
				$dbHmData{$pexp_id}{ratio} =
				  ( $dbHmData{$pexp_id}{ratio} + $pratio ) / 2;
			}
		} else
		{
			$dbHmData{$pexp_id}{locustag} = $locustag;
...
sub xxdbgetheatmapData
{
	...
	my $lTagstring = join( ",", @$ltagRef );
	$sth = $dbh->prepare(
q{ select pexp_id, round(avg(pratio), 3) pratio, locustag from pdata where lower(locustag) in ( select * from THE ( select cast( str2tbl( lower(?) ) as mytableType ) from dual ) ) group by pexp_id, locustag }
	);
	$sth->execute($lTagstring);
	my ( $pexp_id, $pratio, $locustag );
	$sth->bind_columns( \$pexp_id, \$pratio, \$locustag );
...
sub dbgetregulators
{
	my ($ltag) = @_;
	$sql =
qq{ select regulatorgene, effect from sup_regulon where lower(targetlocustag) = lower(?) order by regulatorgene };
	$sth = $dbh->prepare($sql);
	$sth->execute($ltag);
	my %dbregulatorsRec;
...
sub dbgetsigma
{
	my ($ltag) = @_;
	$sql =
qq{ select sigmagene from sup_sigma where lower(targetlocustag) = lower(?) order by sigmagene };
	$sth = $dbh->prepare($sql);
	$sth->execute($ltag);
	my %dbsigmaRec;
	my ($sigmagene);
...
sub dbgetmfunLevels
{
	my ($ltag) = @_;
	$sql =
qq{ select a.blevel,b.mfunction from sup_mfun_bnum a, sup_mfun_levels b where lower(bnum) = lower(?) and a.mid=b.mid order by blevel };
	$sth = $dbh->prepare($sql);
	$sth->execute($ltag);
	my %dbmfunLevelsRec;
	my ( $blevel, $mfunction );
...
sub dbgetpathways
{
	my ($ltag) = @_;
	$sql =
qq{ select b.uniqueid, b.name from sup_pwgenes a, sup_pathways b where lower(a.gene) = lower(?) and a.sup_pathways_id = b.id };
	$sth = $dbh->prepare($sql);
	$sth->execute($ltag);
	my %dbpathwaysRec;
	my ( $uniqueid, $name );
...
sub dbgetgenesInOperon
{
	my ($operon) = @_;
	$sql =
qq{ select gene, locustag, direction from sup_operon where lower(operon) = lower(?) order by gene };
	$sth = $dbh->prepare($sql);
	$sth->execute($operon);
	my %dboperonRec;
...
sub dbgetgenesInRegulon
{
	my ($regulon) = @_;
	$sql =
qq{ select targetgene, targetlocustag, effect, evidence from sup_regulon where lower(regulatorgene) = lower(?) order by targetgene };
	$sth = $dbh->prepare($sql);
	$sth->execute($regulon);
	my %dbregulonRec;
...
sub dbgetgenesInSigma
{
	my ($sigma) = @_;
	$sql =
qq{ select unique targetgene, targetlocustag from sup_sigma where lower(sigmagene) = lower(?) order by targetgene };
	$sth = $dbh->prepare($sql);
	$sth->execute($sigma);
	my %dbsigmaRec;
...
sub dbgetgenesinMfun
{
	my ($mfun) = @_;
	$sql =
qq{ select a.bnum, b.mfunction from sup_mfun_bnum a, sup_mfun_levels b where REGEXP_LIKE( blevel, '^' || lower(?) ) and a.mid= b.mid order by bnum };
	$sth = $dbh->prepare($sql);
	$sth->execute($mfun);
	my %dbmfunRec;
	my ( $bnum, $mfunction );
...
sub dbgetgenesinPway
{
	my ($pway) = @_;
	$sql =
qq{ select a.name,b.gene from sup_pathways a, sup_pwgenes b where lower(a.uniqueid) = lower(?) and a.id=b.sup_pathways_id  };
	$sth = $dbh->prepare($sql);
	$sth->execute($pway);
	my %dbpwayRec;
	my ( $name, $gene );
...
sub dbqryRegulon
{
	my ($query) = @_;
	$sql =
qq{ select targetgene from sup_regulon where lower(regulatorgene) = lower(?) or lower(regulatorlocustag) = lower(?) order by targetlocustag };
	$sth = $dbh->prepare($sql);
...
sub dbqrySigma
{
	my ($query) = @_;
	$sql =
qq{ select targetgene from sup_sigma where lower(sigmagene) = lower(?) order by targetlocustag };
	$sth = $dbh->prepare($sql);
	$sth->execute($query);
	my ( $targetgene, @sigma );
	$sth->bind_columns( \$targetgene );
...
sub dbqryExperiment
{
	...
	$sth = $dbh->prepare($sql);
	$sth->execute( $query, $query, $query, $query, $query, $query, $query,
...
sub dbstatsInfo
{
	...
	$sql =
qq{ select a.cntlgenome from pexp a, curated b where b.status=3 and a.expid=b.expid };
	$sth = $dbh->prepare($sql);
	$sth->execute();
	my $cntlgenome;
	$sth->bind_columns( \$cntlgenome );
	...
	#get sample count
	$sql =
qq{ select b.strain from bioassays_experiment a, curated b where b.status=3 and a.experiment_id=b.expid };
	$sth = $dbh->prepare($sql);
	$sth->execute();
	$sth->bind_columns( \$strain );
	$i = 0;
	...
	#get platform count
	$sql =
qq{ select a.platform,a.cntlgenome from pexp a, curated b where b.status=3 and a.expid=b.expid };
	$sth = $dbh->prepare($sql);
	$sth->execute();
	my $platform;
...
sub dbgetGenomeDBdate
{
	my ($acclist) = @_;
	$sql =
qq{ select accession,adate,organism,sstop from genome where feature='source' and sstart='1' and accession in ( $acclist ) };
	$sth = $dbh->prepare($sql);
	$sth->execute();
...
sub dbplatformInfo
{
	...
	$dbh->{LongReadLen} = 2 * 1024 * 1024;    #2 meg
	$sql =
qq{ select c.identifier, c.name, d.value from curated a, physicalarraydesign b, identifiable c, namevaluetype d where a.status=3 and a.eid=b.eid and b.id=c.id and b.id=d.namevaluetype_id and d.name='Technology' };
	$sth = $dbh->prepare($sql);
	$sth->execute();
...
sub dbplatformCounts
{
	...
	$sql =
	  qq{ select platform, count(*) cnt from platformannot group by platform };
	$sth = $dbh->prepare($sql);
	$sth->execute();
...
{
	$sql =
qq{ select b.identifier accession, b.name expname, c.identifier sample, c.name sampname, d.strain 
	...
	$sth->bind_columns( \$accession, \$expname, \$sample, \$sampname,
						\$strain );
	my $i = 0;
	while ( $row = $sth->fetchrow_arrayref )
	{
...
sub dbexpSamplesInfo
{
	...
	$sql =
qq{ select b.accession, b.expname, b.samples, b.cntlgenome from curated a, pexp b where a.status=3 and a.expid=b.expid };
	$sth = $dbh->prepare($sql);
	$sth->execute();
...
sub dbexpGenesInfo
{
	...
	$sql =
qq{ select c.locustag, count(*) cnt from  curated a, pexp b, pdata c where (a.status=3 and a.expid=b.expid(+)) and b.id=c.pexp_id group by c.locustag order by c.locustag };
	$sth = $dbh->prepare($sql);
	$sth->execute();
...
sub qryNS
{
	...
	$sth = $dbh->prepare(
q{ select a.ltag altag,a.name aname,a.genome agenome,a.type atype,b.ltag bltag,b.name bname,b.genome bgenome,b.type btype from ns_data a, ns_data b where lower(a.name)=lower(?) and a.ltag=b.ltag }
	);
	$sth->execute($query);
	my ( %gene, %ltag, %olt, %ns );
	my ( $altag, $aname, $agenome, $atype, $bltag, $bname, $bgenome, $btype );
	$sth->bind_columns( \$altag, \$aname, \$agenome, \$atype,
						\$bltag, \$bname, \$bgenome, \$btype );
	while ( $row = $sth->fetchrow_arrayref )
	{
		$gene{ lc($aname) }{$agenome}{$altag} = 1
		  if ( $atype =~ /gene|synonym/ );
		$ltag{ lc($aname) }{$agenome}{$altag} = 1 if ( $atype =~ /locustag/ );
		$olt{ lc($aname) }{$agenome}{$altag} = 1
		  if ( $atype =~ /old_locus_tag/ );
		$gene{ lc($bname) }{$bgenome}{$bltag} = 1
		  if ( $btype =~ /gene|synonym/ );
		$ltag{ lc($bname) }{$bgenome}{$bltag} = 1 if ( $btype =~ /locustag/ );
		$olt{ lc($bname) }{$bgenome}{$bltag} = 1
		  if ( $btype =~ /old_locus_tag/ );
	}
	my %tags;
	if ( $gene{$query} )
	{
		for my $ltag ( sort { lc($a) cmp lc($b) }
					   keys %{ $gene{$query}{$genome} } )
		{
			$tags{$ltag} = $genome;
		}
	} elsif ( $ltag{$query} )
	...
	if ( !%tags )
	{
		...
		{
			for
			  my $agenome ( sort { lc($a) cmp lc($b) } keys %{ $ltag{$aname} } )
			{
				if ( $query =~ $aname )
				{
					for my $ltag ( sort { lc($a) cmp lc($b) }
								   keys %{ $ltag{$aname}{$agenome} } )
					{
						$tags{$ltag} = $agenome;
					}
				}
	...
	if ( !%tags )
	{
		if ( ( $genome =~ /NC_000913/ ) and ( $query =~ /^b/i ) )
		{
			$tags{$query} = $genome;
		}
		if ( ( $genome =~ /NC_002655/ ) and ( $query =~ /^z/i ) )
		{
			$tags{$query} = $genome;
		}
		if ( ( $genome =~ /NC_002695/ ) and ( $query =~ /^e/i ) )
		{
			$tags{$query} = $genome;
		}
		if ( ( $genome =~ /NC_004431/ ) and ( $query =~ /^c/i ) )
		{
			$tags{$query} = $genome;
		}
		if ( ( $genome =~ /NC_007946/ ) and ( $query =~ /^u/i ) )
		{
			$tags{$query} = $genome;
		}
		if ( ( $genome =~ /NC_013716/ ) and ( $query =~ /^r/i ) )
		{
			$tags{$query} = $genome;
		}
		if ( ( $genome =~ /NC_003197/ ) and ( $query =~ /^s/i ) )
		{
			$tags{$query} = $genome;
		}
		if ( ( $genome =~ /NC_004631/ ) and ( $query =~ /^t/i ) )
		{
			$tags{$query} = $genome;
		}
		if ( ( $genome =~ /NC_002505/ ) and ( $query =~ /^(vc)+[0-9]{4}/i ) )
		{
			$tags{$query} = $genome;
		}
		if ( ( $genome =~ /NC_002506/ ) and ( $query =~ /^vca/i ) )
		{
			$tags{$query} = $genome;
		}
	}
	...
	my @atags = keys %tags;
	if ( ( scalar @atags ) < 2 )
	{    #we found only 1 (or none) ltag
		my $rel =
		  ( $atags[0] )
		  ? $atags[0]
		  : $query;    #use ltag (if we have one), if not use query
		$sth = $dbh->prepare(
q{ select a.ltag altag,a.name aname,a.genome agenome,a.type atype,b.ltag bltag,b.name bname,b.genome bgenome,b.type btype from ns_data a, ns_data b where lower(a.name)=lower(?) and a.ltag=b.ltag }
		);
		$sth->execute($rel);
		my ( $altag, $aname, $agenome, $atype, $bltag, $bname, $bgenome,
			 $btype );
		$sth->bind_columns( \$altag, \$aname, \$agenome, \$atype,
							\$bltag, \$bname, \$bgenome, \$btype );
		while ( $row = $sth->fetchrow_arrayref )
...
sub dbgetCurated
{
	...
	{
		$sql =
qq{ select accession, status, pmid, strain, substrain, info, geodesc, geomatch, adddate, adduser, moddate, moduser from curated where accession = ? };
		$sth = $dbh->prepare($sql);
		$sth->execute($acc);
	} else
	{
		$sql =
qq{ select accession, status, pmid, strain, substrain, info, geodesc, geomatch, adddate, adduser, moddate, moduser from curated };
		$sth = $dbh->prepare($sql);
		$sth->execute();
	}
	my (
		 $accession, $status,  $pmid,     $strain,  $substrain,
		 $info,      $geodesc, $geomatch, $adddate, $adduser,
		 $moddate,   $moduser, %curated
	);
	$sth->bind_columns(
						\$accession, \$status,  \$pmid,    \$strain,
						\$substrain, \$info,    \$geodesc, \$geomatch,
...
sub dbputgeoUpdate
{
	...
	$sql =
qq{ insert into curated (id, accession, geodesc, geomatch, status, adddate, adduser)  values ( id_seq.nextval, ?, ?, ?, 1, sysdate, ? ) };
	$sth = $dbh->prepare($sql);
	my $numadded = 0;
	for my $acc ( keys %gse )
	{
		...
		{
			$numadded++;
			$sth->execute( $acc, $gse{$acc}{desc}, $gse{$acc}{match},
						   $username );
		}
	}
	return $numadded;
...
sub dbgetExpMean
{
	my ($expid) = @_;
	$sql =
qq{ select avg(pratio) mean from pdata where pexp_id=? and pratio is not null };
	$sth = $dbh->prepare($sql);
	$sth->execute($expid);
	my ($mean);
