...
sub savepexp {
	...
	while ( $row = $sth->fetchrow_arrayref ) {
		...
		$accCnts{$accession}++;
		print PEXPPARMS "$id\t$expname\t$accession\t$samples\t$channels\t$testcolumn\t$testbkgd\t$controlcolumn\t$cntlbkgd\t$logarithm\t$normalize\t$antilog\t$userma\t$plottype\t$info\t$timepoint\t$exporder\t$platform\t$testgenome\t$cntlgenome\t$ORGANISM\t$STRAIN\t$MUTANT\t$CONDITION\t$SOURCE\n";
	}
	close(PEXPPARMS);
...
#return;		
	...
	$sql = qq{ insert into pexp (id,eid,expname,expid,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,timepoint,expstddev,exporder,platform,testgenome,cntlgenome,adddate,adduser,ORGANISM,STRAIN,MUTANT,CONDITION,SOURCE) values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,sysdate,?, ?,?,?,?,? ) };
