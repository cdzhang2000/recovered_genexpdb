...
my $host =
  "genexpdb-dev.ccrlikknzibd.us-east-1.rds.amazonaws.com";
my $sid           = "GENEXPDB";
...
#$sql = qq{ select id, expname,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,timepoint,exporder,platform,testgenome,cntlgenome, ORGANISM, STRAIN, MUTANT, CONDITION, SOURCE from pexp order by to_number(substr(accession,4)),exporder,timepoint,samples };
#czhang query pexp data based on where clause
#$sql = qq{ select id, expname,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,timepoint,exporder,platform,testgenome,cntlgenome,  from pexp where accession in (select accession from pexp where to_char(adddate, 'yyyymmdd') > '20120923') order by to_number(substr(accession,4)),exporder,timepoint,samples };
	$sql =
qq{ select id, expname,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,timepoint,exporder,platform,testgenome,cntlgenome, ORGANISM,STRAIN,MUTANT,CONDITION,SOURCE from pexp where accession='GSE3123' order by to_number(substr(accession,4)),exporder,timepoint,samples };
	$sth = $dbh->prepare($sql);
	$sth->execute();
	my (
	...
	while ( $row = $sth->fetchrow_arrayref )
	{
		...
		$accCnts{$accession}++;
		print PEXPPARMS
"$id\t$expname\t$accession\t$samples\t$channels\t$testcolumn\t$testbkgd\t$controlcolumn\t$cntlbkgd\t$logarithm\t$normalize\t$antilog\t$userma\t$plottype\t$info\t$timepoint\t$exporder\t$platform\t$testgenome\t$cntlgenome\t$ORGANISM\t$STRAIN\t$MUTANT\t$CONDITION\t$SOURCE\n";
	}
	close(PEXPPARMS);
	...
	for my $accession ( sort { substr( $a, 3 ) <=> substr( $b, 3 ) }
						keys %accCnts )
	{
...
sub c1data
{
	...
	my $dbsampInfoRef = testgdb::oracle::dbsampInfo($expid);
...
qq{<font color="red">Cannot open Test sample file $testsample{$bioassays_id}{sampid} !!</font>};
		...
		foreach my $line (@data)
		{
			...
			if ( $datacol < 0 )
			{
				...
				my ($labelRef) = testgdb::oracle::dbsampFilehead($bioassays_id);
				my %label = %$labelRef;
		...
		for my $i ( sort { $a <=> $b } keys %pfids )
		{
			...
			next
			  ...
			  ;    #Tiling platform, skip multiple ltags per id_ref
...
#$sql = qq{ insert into pexp (id,eid,expname,expid,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,timepoint,expstddev,exporder,platform,testgenome,cntlgenome,adddate,adduser) values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,sysdate,? ) };
	$sql =
qq{ insert into pexp (id,eid,expname,expid,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,timepoint,expstddev,exporder,platform,testgenome,cntlgenome,adddate,adduser,ORGANISM,STRAIN,MUTANT,CONDITION,SOURCE) values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,sysdate,?, ?,?,?,?,? ) };
	$sth = $dbh->prepare($sql);
	my $pexp_id = testgdb::oracle::dbgetPdataNextSeq();
	if ( !$pexp_id or !$eid )
	{
...
qq{ insert into pdata (id, eid, pexp_id, pavg, pratio, locustag, multid ) values ( ?, ?, ?, ?, ?, ?, ? ) };
	...
	my $geneLocRef =
	  testgdb::oracle::dbgetGeneLoc($cntlgenomeacc);    #genome gene and start
	my %geneLoc      = %$geneLocRef;
	my $NSgeneLocRef = testgdb::oracle::dbgetNSgeneLoc($cntlgenomeacc);
	...
	for my $id_ref ( sort keys %plotCntldata )
	{
		...
		foreach my $ltag (@ltags)
		{
			...
			} else
			{
				...
				{
					$gene = $geneLoc{ $NSgeneLoc{$ltag} }{gene}
					  if $geneLoc{ $NSgeneLoc{$ltag} }{gene};
				}
			}
			my $pdata_id = testgdb::oracle::dbgetPdataNextSeq();
			if ( !$pdata_id )
			{
...
sub c2data
{
	...
	for my $bioassays_id ( sort keys %sample )
	{
		...
		foreach my $line (@data)
		{
			...
			#check if the id is in the platform annotation file
			if ( exists $dbplatformAnnot{$id} )
			{

				for my $ltag ( keys %{ $dbplatformAnnot{$id} } )
				{
					#put id ref into one string, sparete them with :
					$id_ref .= ":$ltag";    #all genomes
				}
			}
		...
		for my $i ( sort { $a <=> $b } keys %pfids )
		{
			my $id_ref = $pfids{$i}{ltag};
			next
			  if ( $id_ref =~ /:/ and $platform =~ /GPL7714/ )  ;    #Tiling platform, skip multiple ltags per id_ref
...
#1 or more samples have been selected.  We will plot each sample and save values so we can average the final combined plot
		...
		$stddev = sprintf( "%.3f", testgdb::plot::stat_stdev( \@stddata ) );
	}    #end of all samples
...
#save data to pdata
	...
	my $geneLocRef =  testgdb::oracle::dbgetGeneLoc($cntlgenomeacc);    #genome gene and start
	my %geneLoc      = %$geneLocRef;
