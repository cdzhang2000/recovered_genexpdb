...
# we need to delete the existing Experiment PEXP rec and PDATA recs before we add new ones.
	...
	my $pexp_id = testgdb::oracle::dbgetPexpNextSeq();
	if ( !$pexp_id or !$eid )
	{
...
#save data to pdata
	my $sqldata =qq{ insert into pdata (eid, pexp_id, pavg, pratio, locustag ) values ( ?, ?, ?, ?, ?) };
...
sub delExistingExpm
{
	...
	#only delete a specific experiment every time
	my $sthdata = $dbh->prepare($sqldata);
