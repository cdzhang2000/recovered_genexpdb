...
qq{ select id, expname,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,timepoint,exporder,platform,testgenome,cntlgenome from pexp where (accession= 'GSE20374' OR accession= 'GSE20397') order by to_number(substr(accession,4)),exporder,timepoint,samples };
	...
	while ( $row = $sth->fetchrow_arrayref )
	{
		...
		$accCnts{$accession}++;
		print PEXPPARMS
"$id\t$expname\t$accession\t$samples\t$channels\t$testcolumn\t$testbkgd\t$controlcolumn\t$cntlbkgd\t$logarithm\t$normalize\t$antilog\t$userma\t$plottype\t$info\t$timepoint\t$exporder\t$platform\t$testgenome\t$cntlgenome\n";
	}
	close(PEXPPARMS);
...
# we need to delete the existing Experiment PEXP rec and PDATA recs before we add new ones.
	...
	#save info to pexp
	$sql =
qq{ insert into pexp (id,eid,expname,expid,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,timepoint,expstddev,exporder,platform,testgenome,cntlgenome,adddate,adduser) values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,sysdate,? ) };
	$sth = $dbh->prepare($sql);
