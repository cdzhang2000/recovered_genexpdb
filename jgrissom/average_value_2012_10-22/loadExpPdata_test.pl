...
{
	$sql =
qq{ select id, expname,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,timepoint,exporder,platform,testgenome,cntlgenome from pexp order by to_number(substr(accession,4)),exporder,timepoint,samples };
	$sth = $dbh->prepare($sql);
	$sth->execute();
	my (
...
##for testing
	#print "\t$stddev\n";
	#return;
	my $gcnt = scalar keys %plotCntldata;
	print "\tSTD:$stddev\tCNT:$gcnt\t";
	##================================================================================================================================
