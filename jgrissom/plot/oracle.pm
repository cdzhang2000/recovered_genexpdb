...
sub dbgetGeneLocBasedPlatform {
	...
	$sql = qq{ select unique g.locus_tag, gene, sstart from genome g, PLATFORMANNOT p where PLATFORM=? and p.locustag=g.locus_tag};
	$sth = $dbh->prepare($sql);
...
sub dbgetGeneStartLoc {
	...
	$sql = qq{ select unique locus_tag, sstart from genome where accession=? and locus_tag is not null order by to_number(sstart)};
	$sth = $dbh->prepare($sql);
	$sth->execute($genome);
