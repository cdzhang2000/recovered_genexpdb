...
#
# Description : 
# 1) query GSEs from curated_test table
# 2) /var/www/modperl/geoLoad$ perl ./UpdateCuratedColumn.pl updateColumn 
# 3) update curated t1 set (institution, author) =(select institution, author from curated_test t2
# where t2.accession = t1.accession) where exists (select * from curated_test t2  where t2.accession = t1.accession)
#
# Author      : czhang
...
sub update
{
	...
	$sth->execute();  #update curated_test table		
