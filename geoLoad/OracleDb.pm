...
sub dbUpdateCurated {
	...
	$sql = qq{ update curated set eid = ?, status=3, moddate=sysdate, moduser='czhang' where accession = ? };
	$sth = $dbh->prepare($sql);
...
#----------------------------------------------------------------------
# check if GSE is already in the curated table
# input: none
# return: hash
#----------------------------------------------------------------------
sub checkAccesion{
    ...
    my $accession=$_[0];   
	my $sql = qq{ select status from curated where accession = ?};
...
#----------------------------------------------------------------------
# get status==3 accessions
# input: none
# return: hash
#----------------------------------------------------------------------
sub getAccessions{

	$sql = qq{ select accession from curated where status=3 and (institution is null or email is null or PI is null) order by to_number(substr(accession,4)) };
	$sth = $dbh->prepare($sql);
	$sth->execute();
...
#----------------------------------------------------------------------
# get pending GSEs from CURATED_TEST table
# input: none
# return: hash
# czhang Nov , 2013 
#----------------------------------------------------------------------
sub getPendingGSEs{
	...
	$sql = qq{ select accession from curated_test where status=2 order by to_number(substr(accession,4)) };
	$sth = $dbh->prepare($sql);
	$sth->execute();
	my ($accession);
	$sth->bind_columns( \$accession );
