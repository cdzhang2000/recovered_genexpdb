...
sub dbpearsonCorr
{
	...
	my @newqry    = testgdb::heatmap::ckQryLoc( $pcorrgene, $genomeacc )
...
sub dbAccessionsInfo
{
	...
	#get the organism passed from acession.pm
	my $search_org = $_[0];
	if ($search_org)
	{
...
sub xxx_dbplatformAnnot
{
	...
	foreach my $val (@$vals)
	{
		...
		#increment by 1, will support multi-IDs
		$dbplatformAnnot{ lc($id_ref) }{$locustag}++;
	}
	return ( \%dbplatformAnnot );
}
...
qq{ insert into pexp (id,eid,expname,expid,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,expstddev,platform,testgenome,cntlgenome,adddate,adduser, expmean) values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,SYSTIMESTAMP,?, ?) };
	...
	if (
		 !$sth->execute(
						 $pexp_id,
						 $expEID,
						 $testgdb::webUtil::frmData{expname},
						 $savExpInfo{expid},
...
qq{ insert into pexp (id,eid,expname,expid,accession,samples,channels,testcolumn,testbkgd,controlcolumn,cntlbkgd,logarithm,normalize,antilog,userma,plottype,info,expstddev,platform,testgenome,cntlgenome,adddate,adduser, expmean) values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,SYSTIMESTAMP,?, ?) };
	...
	} else
	{
		...
		close(FILE);
		shift(@data);    #remove heading line
		open MYFILE, ">/var/www/modperl/testgdb/pdata/pdata.txt"
		...
		close MYFILE;
		my $DB =
"genexpdb/vb1g3n3xpdb\@genexpdb-dev.ccrlikknzibd.us-east-1.rds.amazonaws.com:3306/GENEXPDB";
		my $pdata_ctl = "/var/www/modperl/testgdb/pdata/pdata.ctl";
		my $pdata_dat = "/var/www/modperl/testgdb/pdata/pdata.txt";
		my $pdata_log = "/var/www/modperl/testgdb/pdata/pdata.log";
		my $pdata_bad = "/var/www/modperl/testgdb/pdata/pdata.bad";
...
{
	my ( $savExpInfoRef, $organism, $strain, $mutant, $condition, $timepoint ) =
	  @_;
	my %savExpInfo = %$savExpInfoRef;
	my $pexp_id    = dbgetPdataNextSeq();
	my $expEID     = dbgetExpEID( $savExpInfo{expid} );
...
#Check if the experiment has been already created. 
#A warning message is shown if it's already in the database.
#
