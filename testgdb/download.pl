...
if ($frmData{type} =~ /metadata/) {
	downloadmetadata( \%frmData );
}
...
sub downloadmetadata{
	...
	for my $i ( sort { $a <=> $b } keys %dbExpmRec ) {
		...
		if ( exists $downloadid{$accid} ) {
			...
			$accinfo{$i}{organism}  = $dbExpmRec{$i}{organism};
			$accinfo{$i}{strain}    = $dbExpmRec{$i}{strain};
			$accinfo{$i}{mutant}    = $dbExpmRec{$i}{mutant};
			$accinfo{$i}{source}    = $dbExpmRec{$i}{source};			
			$accinfo{$i}{condition} = $dbExpmRec{$i}{condition};
	...
	for my $i ( sort { $a <=> $b } keys %accinfo ) {
		...
		$exp{ $accinfo{$i}{expid} }{ $accinfo{$i}{id} }{organism}        = $accinfo{$i}{organism};
		$exp{ $accinfo{$i}{expid} }{ $accinfo{$i}{id} }{strain}        = $accinfo{$i}{strain};
		$exp{ $accinfo{$i}{expid} }{ $accinfo{$i}{id} }{mutant}        = $accinfo{$i}{mutant};
		$exp{ $accinfo{$i}{expid} }{ $accinfo{$i}{id} }{source}        = $accinfo{$i}{source};
		$exp{ $accinfo{$i}{expid} }{ $accinfo{$i}{id} }{condition}        = $accinfo{$i}{condition};
	...
	my ( $dlfile);
		...
		#one accession
		$dlfile .= "ACCESSION\tEXP\tNAME\tSTDDEV\tSAMPLES\tTIMEPOINT\tCHANNELS\tTEST COL\tTEST BKGD\tCNTL COL\tCNTL BKGD\tLOG\tNORM\tRMA DATA\tORGANISM\tSTRAIN\tMUTANT\tCONDITION\tSOURCE\n";
		
		foreach my $expid (@accOrd) {					
			#$dlfile .= "ACCESSION\tEXP\tNAME\tSTDDEV\tSAMPLES\tTIMEPOINT\tCHANNELS\tTEST COL\tTEST BKGD\tCNTL COL\tCNTL BKGD\tLOG\tNORM\tRMA DATA\n";
			foreach my $tmp (@exoOrd) {
				my ( $eid, $id ) = split( /:/, $tmp );
				if ( $eid == $expid ) {
					$dlfile .= "$acc{$expid}{accession}\t$exp{$expid}{$id}{ouid}\t$exp{$expid}{$id}{expname}\t$exp{$expid}{$id}{std}";
					$dlfile .= "\t$exp{$expid}{$id}{samples}\t$exp{$expid}{$id}{timepoint}\t$exp{$expid}{$id}{channels}\t$exp{$expid}{$id}{testcolumn}";
					$dlfile .= "\t$exp{$expid}{$id}{testbkgd}\t$exp{$expid}{$id}{controlcolumn}\t$exp{$expid}{$id}{cntlbkgd}\t$exp{$expid}{$id}{logarithm}";
					$dlfile .= "\t$exp{$expid}{$id}{normalize}\t$exp{$expid}{$id}{userma}\t$exp{$expid}{$id}{organism}\t$exp{$expid}{$id}{strain}\t$exp{$expid}{$id}{mutant}\t$exp{$expid}{$id}{condition}\t$exp{$expid}{$id}{source}";
					$dlfile .= "\n";
				}
			}		
		}
