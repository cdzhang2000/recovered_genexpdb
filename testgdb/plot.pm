...
#----------------------------------------------------------------------
sub createExpPlot {
	my ( $plottype, $stddev, $genomeacc, $plotTestdataRef, $plotCntldataRef, $platform ) = @_;
	...
	my $geneLocRef = testgdb::oracle::dbgetGeneLocBasedPlatform($platform);	#genome gene and start
	my %geneLoc    = %$geneLocRef;
...
sub expplot {
		...
		#
        my $type   = ( $testgdb::webUtil::frmData{type} )    ? $testgdb::webUtil::frmData{type}    : '';
        
        my $expmid   = ( $testgdb::webUtil::frmData{expmid} )    ? $testgdb::webUtil::frmData{expmid}    : '';
        my $accessionid   = ( $testgdb::webUtil::frmData{accnid} )    ? $testgdb::webUtil::frmData{accnid}    : '';
        my $nsd     = ( $testgdb::webUtil::frmData{nsd} =~ /undefined/ ) ? 2 : $testgdb::webUtil::frmData{nsd};
        ...
        my $expmean = testgdb::oracle::dbgetExpMean($expid);    #get experiment mean
