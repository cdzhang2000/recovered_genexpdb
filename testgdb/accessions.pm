#------------------------------------------------------------------------------------------
# FileName    : testgdb/accessions.pm
#
...
sub displayAccessions
{
		 ...
		 and $testgdb::webUtil::frmData{ginfo} =~ /^accessions/ )
	{
		...
		print " parms->{accessions} = $parms->{accessions}";
...
sub accessions
{
	...
	my $dbAccessionRec = testgdb::oracle::dbAccessionsInfo($organism);
	my %dbAccession    = ();
...
qq{<td class="tdl" onmouseover="return overlib('$institution');" onmouseout="return nd();">$tmp<b>...</b></td>\n};
		...
		{
			#czhang Dec 10, 2013
			 $institution =" " unless defined $institution;
...
sub accExperiments
{
	...
	my %acchm    = ();
	%acchm = %$acchmRef if $acchmRef;
	my ( $dbaccExpmRec, $expm_orderRef ) = testgdb::oracle::dbgetAccExpm()
...
qq{<td valign="top"><a class="exmp" onclick="accinfo('accinfo',$testgdb::webUtil::frmData{id});" onmouseover="this.style.cursor='pointer';return overlib('click to close');" onmouseout="return nd();">close</a></td>\n},
	  qq{<td>\n},
	  qq{<table class="tblb">\n}, qq{<tr>\n},
qq{<th class="thc" colspan="10">$row->{ACCESSION} Accession Information</th>\n},
	  qq{</tr>\n},
	  qq{<tr>\n},
...
qq{<div class="hidden" id="sampinfo$testgdb::webUtil::frmData{id}"></div>\n},
	  ...
	  #czhang
qq{<div class="hidden" id="expplot$testgdb::webUtil::frmData{id}"></div>\n},
...
sub expinfo
{
	...
	my ( $dbExpmRef, $expmorderRef ) = testgdb::oracle::dbgetExpmInfo_VBI($expid);    #all experiments, needed for ouid
	my %dbExpm       = %$dbExpmRef;
	
	#my $expid        = $testgdb::webUtil::frmData{id};
	my $dbexpInfoRef = testgdb::oracle::dbexpInfo_VBI($expid);
...
qq{<tr><td class="tdc" colspan="3"><input class="ebtn" type="button" value="Update" onclick="updexperiment($expid,this.form)"></td><td class="small">$status</td></tr>\n}
	...
	#print "<pre>" . Dumper( %dbexpInfo ) . "</pre>";
...
sub updexperiment
{
	...
	for my $id ( sort { $a <=> $b } keys %dbexpInfo )
	{
		if ( $delexp[$id] =~ /true/ )
		{
			...
			#delete pdata table first and then pexp table because of foreign key
...
sub updcurated
{
	...
	#parse the form value and setup all values
	my (
	...
	$info =~ s/\r//g if ( defined $info );
	$change .= "info='$info',"
	  if ( $dbcuratedInfo{info} ne ( defined $info ? $info : "" ) );
	my $status;
	print "change=" . $change . "\n\n";

	if ($change)
	{
		my $rc = testgdb::oracle::dbupdatecuratedInfo(
...
sub expdata
{
	...
	my $dbchannelCountsRef = testgdb::oracle::dbchannelCounts_vbi($selid);
...
qq{<tr><td class="tdl"><b>PLOT INFORMATION</b> ($numchannel channel) $chgNumchan</td></tr>\n},
	 ...
	 # if ( $numchannel == 1 )
	#	{
	#print qq{<b>Data Process:</b> <input type="checkbox" id="log$selid" value="1">Log(base2) &nbsp; <input type="checkbox" id="normalize$selid" value="1">Normalize(loess) &nbsp; <input type="checkbox" id="antilog$selid" value="1">Anti-Log10 &nbsp; <input type="checkbox" id="antilog2$selid" value="1">Anti-Log2\n};
	#	}else{
	#print	qq{<b>Data Process:</b> <input type="checkbox" id="log$selid" value="1">Log(base2) &nbsp; <input type="checkbox" id="normalize$selid" value="1">Normalize(loess) &nbsp; <input type="checkbox" id="antilog$selid" value="1">Anti-Log10\n};	
...
qq{<font color="red">No Platform available!  Please contact administrator.</font>};
	...
	#CZHANG Feb 20, 2013
	my $antilog2 = ( $testgdb::webUtil::frmData{antilog2} =~ /true/i ) ? 1 : 0;
...
qq{<input class="ebtn" type="button" value="Download data" onclick="window.open('$testgdb::util::urlpath/download.pl?type=plotdata&fname=$dataFilename&samp=Test($avgTestsname) / Control($avgCntlsname)','_blank')" onmouseover="this.style.cursor='pointer';return overlib('Download data');" onmouseout="return nd();">\n},
	...
	if ( $testgdb::webUtil::useracclevel > 2 )
	{
		...
		my ( $vbi_organism, $vbi_strain, $vbi_mutant, $vbi_cond,
			 $vbi_timepoint );
...
qq{<td onmouseover="this.style.cursor='pointer';return overlib('Swap ExpName conditions. (Test / Control)');" onmouseout="return nd();"><input class="ebtn" type="button" value="Swap Conditions" onclick="swapcond($testgdb::webUtil::frmData{id})"></td>},
		  ...
		  #czhang
		  qq{<tr>}, qq{<td class="tdr"><b>Organism:</b></td>},
		  ##jgrissom_27Nov2012
#   qq{<td> <input class="small" type="text" size="110" maxlength="250" id="vbi_organism$inputid" value="$vbi_organism"> </td>},
qq{<td> <input class="small" type="text" size="110" maxlength="250" id="vbi_organism$testgdb::webUtil::frmData{id}" value="$vbi_organism"> </td>},
		  qq{</tr>},
		  qq{<tr>},
...
qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_strain$testgdb::webUtil::frmData{id}" value="$vbi_strain"> </td>},
		  qq{</tr>},
		  qq{<tr>},
...
qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_mutant$testgdb::webUtil::frmData{id}" value="$vbi_mutant"> </td>},
		  qq{</tr>},
		  qq{<tr>},
...
qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_cond$testgdb::webUtil::frmData{id}" value="$vbi_cond"> </td>},
		  qq{</tr>},
		  qq{<tr>},
...
qq{<td><input class="small" type="text" size="110" maxlength="250" id="vbi_timepoint$testgdb::webUtil::frmData{id}" value="$vbi_timepoint"></td>},
		  qq{</tr>},
		  qq{<tr>},
...
qq{<td class="tdl"><textarea class="small" id="info$testgdb::webUtil::frmData{id}" rows="3" cols="110"></textarea></td>},
		  qq{</tr>},
		  qq{<tr>}, qq{<td></td>\n},
qq{<td><input class="ebtn" type="button" value="Save Experiment" onclick="savExptoDB_2(event,$testgdb::webUtil::frmData{id},$testgdb::webUtil::frmData{id})"></td>\n},
		  qq{</tr>},
		  qq{</table>};
...
"$testgdb::util::datapath/$sample{$bioassays_id}{accession}/$sample{$bioassays_id}{fname}"
		...
		foreach my $line (@data)
		{
			...
			#czhang $id is id_ref in sample expression value file
			my $id = $lineArr[0];
			$id =~ s/^\s+//;
			$id =~ s/\s+$//;
			$id = lc($id);
...
sub savExp
{
	...
	#my $expname = '';
	my (
		 $expname,    $vbi_organism, $vbi_strain,
		 $vbi_mutant, $vbi_cond,     $vbi_timepoint
	);
	$expname = $vbi_organism = $vbi_strain = $vbi_mutant = $vbi_cond =
	  $vbi_timepoint = "";
	...
	print qq{<table align="center" cellpadding="1"  cellspacing="1">\n},
	  qq{<tr>}, qq{<td class="tdr"><b>ExpName:</b></td>},
qq{<td><input class="small" type="text" size="110" maxlength="250" id="expname$inputid" value="$expname"></td>},
...
sub savExptoDB
{
	...
	my $expname       = $testgdb::webUtil::frmData{expname};
	my $vbi_organism  = $testgdb::webUtil::frmData{vbi_organism};
	my $vbi_strain    = $testgdb::webUtil::frmData{vbi_strain};
	my $vbi_cond      = $testgdb::webUtil::frmData{vbi_cond};
	my $vbi_mutant    = $testgdb::webUtil::frmData{vbi_mutant};
	my $vbi_timepoint = $testgdb::webUtil::frmData{vbi_timepoint};
	if ( !$vbi_organism )
	{
		$vbi_organism = "";
	} 
	...
	#czhang vbi func
	my $exprows = testgdb::oracle::checkExpDuplicate($savExpInfoRef);
	...
	if ( $exprows > 0 )
	{
		...
		my $rc =
		  testgdb::oracle::dbsaveExptoDB_vbi3(
