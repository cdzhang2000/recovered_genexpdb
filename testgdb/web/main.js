...
function download() {
	alert('download data is clicked');
...
function downloadMetadata() {
	alert('download metadata is clicked');
	var ckdlaccn='',ckdlexpm='';
	for (var i=0; i < document.mainFrm.ckdlaccn.length; i++) {
		if (document.mainFrm.ckdlaccn[i].checked) {
			ckdlaccn = ckdlaccn + document.mainFrm.ckdlaccn[i].value + ',';
	...
	window.open('/modperl/testgdb/download.pl?type=metadata&ckdlaccn='+ckdlaccn+'&ckdlexpm='+ckdlexpm);
}
...
function selPlot1(id) {
	...
	//czhang Feb 20, 2013
	var antilog2=document.getElementById('antilog2'+id).checked;
...
//	if (cntlgenome.length <1) {alert('Control Genome must be selected!');return false;}
	...
	rec.innerHTML = '<div align="center"><img src="/modperl/testgdb/web/running.gif" alt=""></div>';
	sendRequest('/testgdb/', 'ajax=accinfo&accinfo=sel1Plot&id='+id+'&log='+log+'&normalize='+normalize+'&antilog='+antilog+'&antilog2='+antilog2+'&userma='+userma+'&plottype='+plottype+'&testname='+testnameArr+'&testgenome='+testgenome+'&cntlname='+cntlnameArr+'&cntlgenome='+cntlgenome+'&datacol='+datacol);
	rec.className = 'showrec';
}
...
//	if (cntlgenome.length <1) {alert('Genome must be selected!');return false;}
	...
	rec = document.getElementById('selPlot'+id);
	rec.innerHTML = '<div align="center"><img src="/modperl/testgdb/web/running.gif" alt=""></div>';
	sendRequest('/testgdb/', 'ajax=accinfo&accinfo=sel2Plot&id='+id+'&log='+log+'&normalize='+normalize+'&antilog='+antilog+'&antilog2='+antilog2+'&plottype='+plottype+'&sampname='+sampnameArr+'&cntlgenome='+cntlgenome+'&testcol='+testcol+'&testbkgd='+testbkgd+'&cntlcol='+cntlcol+'&cntlbkgd='+cntlbkgd);
	rec.className = 'showrec';
}
...
function savExptoDB(event,selid,inputid) {
	...
	//support metadata from accession.pm savExp  ##jgrissom_27Nov2012
	var vbi_organism=document.getElementById('vbi_organism'+inputid).value.replace(/^\s+|\s+$/g,'');
	var vbi_strain=document.getElementById('vbi_strain'+inputid).value.replace(/^\s+|\s+$/g,'');
	var vbi_mutant=document.getElementById('vbi_mutant'+inputid).value.replace(/^\s+|\s+$/g,'');
	var vbi_cond=document.getElementById('vbi_cond'+inputid).value.replace(/^\s+|\s+$/g,'');
	var vbi_timepoint=document.getElementById('vbi_timepoint'+inputid).value.replace(/^\s+|\s+$/g,'');
	
	//if you use chrome, this will output to the console log.
	//console.log(vbi_organism); 
	
	rec = document.getElementById('selPlot'+selid);
	rec.innerHTML = '<div align="center"><img src="/modperl/testgdb/web/running.gif" alt=""></div>';
	//sendRequest('/testgdb/', 'ajax=accinfo&accinfo=savExptoDB&expname='+expname+'&info='+info);
	sendRequest('/testgdb/', 'ajax=accinfo&accinfo=savExptoDB&expname='+expname+'&info='+info+'&vbi_organism='+vbi_organism+'&vbi_strain='+vbi_strain+'&vbi_mutant='+vbi_mutant+'&vbi_cond='+vbi_cond+'&vbi_timepoint='+vbi_timepoint);
	
	rec.className = 'showrec';
}

function savExptoDB_2(event,selid,inputid){
...
//aded function for ploting expression values
function expplot(type,expmid,accnid,nsd) {
        ...
        sendRequest('/testgdb/', 'ajax=plot&plot=expplot&type='+type+'&expmid='+expmid+'&accnid='+accnid+'&nsd='+nsd);
        rec.className = 'showrec';
}
