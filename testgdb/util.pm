...
our %gnomacc = ( "NC_000913", 1, "NC_002655", 2, "NC_002695", 3, "NC_004431", 4, "NC_007946", 5);
...
sub mainMain {
	...
	#does not display genome browser and annotation
	...
	#czhang: passing organism name
	
	testgdb::accessions::displayAccessions($qryltagRef);
...
sub ajax {
	...
	if ( $testgdb::webUtil::frmData{ginfo} ) {
		...
		#czhang July 12,2013
		testgdb::info::downloadExpMetaData( ) if ( $testgdb::webUtil::frmData{ginfo} =~ /^downloadExpMetaData/ );
	...
	if ( $testgdb::webUtil::frmData{plot} ) {
		...
		#czhang added for ploting expression value

		 testgdb::plot::expplot() if ( $testgdb::webUtil::frmData{plot} =~ /^expplot/ );
	...
	if ( $testgdb::webUtil::frmData{accinfo} ) {
		...
		#czhang
		#if ( $testgdb::webUtil::frmData{accinfo} =~ /^savExptoDB/ ){
			
		#	$parms->{vbi_organism} = $parms->{$testgdb::webUtil::frmData{vbi_organism}};
		#	$parms->{vbi_strain} = $parms->{$testgdb::webUtil::frmData{vbi_strain}};
			
		#	testgdb::webUtil::putSessVar( 'parms', $parms );
				
		#	testgdb::accessions::savExptoDB();
		#}
		#end here
...
sub heading {
	...
	print
	  ...
	  qq{<tr>\n}, qq{<td class="hdln1">Welcome to the Gene Expression Database (GenExpDB)</td>\n}, qq{</tr>\n},
...
#put only numbers into the number array
sub transformArray{
	my @nums=@_;
	my @numArray=();
